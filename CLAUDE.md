...
- **Source-data freshness UI** (two separate pieces, `renderSourceHealthBanner` in `static/app.js`, driven by `GET /api/source_health` in `app.py`):
  1. **Always-visible header timestamps:** `#last-updated` shows **"Updated: … ET"** (last `scans.scan_time`, set by `loadScanStatus`) and the adjacent `#app-build` shows **"Build: … ET"** (`build_time` = git HEAD commit time via `_compute_build_time()` in `app.py`, falls back to newest core-file mtime). Both always shown in the header.
