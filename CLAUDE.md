...
- **Builder engine (`static/app.js`):** `PORTFOLIO_FILTER_FIELDS` is the field registry — each entry `{key, label, type:'select'|'text', value:p=>…}` (Program, Level, Credential, College, Campus, In CIM, CIM Step, CIM Change, SVT Status, SVT Sub-status, Speed to Market, GLS Status, OTP Status, Inactivation of Admission, Admitting Today, Notes). The filter is a recursive tree `{type:'group', conj:'all'|'any', children:[…]}` where children are rules `{type:'rule', field, op, value}` or nested groups. `evalPortfolioNode`/`evalPortfolioRule` evaluate it; operators by type — select: `in` ("is one of") / `not_in` / `is_set` / `is_empty`; text: `contains` / `equals` / `starts_with` / `is_set` / `is_empty`. `getPortfolioFieldValues(key)` builds the distinct value list for select popups. The applied tree lives in `portfolioFilterTree` and is ANDed into `getPortfolioFiltered()` *after* the top-bar quick filters (the two are independent layers — a view captures both).
- **Modal (`openPortfolioViewsModal`):** header shows a live "N programs match" count (`#pv-modal-count`); sidebar = `+ New view` / Team views (built-in `All` + `GTM`, plus saved `team_*`) / My views (`personal_*`), each clickable (loads + applies) with a delete × on saved ones; main panel = recursive `_renderPvGroup`/`_renderPvRule`/`_renderPvRuleValue` (Match all/any + `+ Add rule` / `⊕ Add nested group` + multi-select chip popups); footer = `Clear all` · `Editing: <name>` · `Cancel` · `↻ Update "<name>"` (personal/team only) · `+ Save as My View` · `+ Save as Team View`. Edits apply live (the draft tree IS `portfolioFilterTree`). Path-based mutators: `pvbAddRule`/`pvbAddGroup`/`pvbRemove`/`pvbSetConj`/`pvbSetField`/`pvbSetOp`/`pvbSetValue`/`pvbToggleMulti`/`pvbOpenMulti` via `_pvWalk` (index paths like `"0.2.1"`).
...
- **Static site:** team views are baked into `data.json` (`team_views` key) by `export_static.py`; the static `loadPortfolioDashboard` override hydrates `portfolioTeamViews` from the payload and restores the saved active view. Save/Update/Delete are local-only on the static site (`window._staticMode` short-circuits `_persistTeamViews`).
