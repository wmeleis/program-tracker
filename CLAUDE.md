...
**CIM effective term (`eff_term`).** CIM's program XML carries `<eff_term>` (a Banner term code, e.g. `202710`) in addition to `<eff_cat>`. It was read every scan but **only persisted for courses** — for programs it was dropped. Now persisted: `programs.eff_term` (scraper writes it at every program_data site; migration adds the column) → `portfolio_programs.cim_eff_term` (ingest, all CIM rows) → shown as the **"CIM Effective Term"** column (`cimterm`) with a best-effort decode (`_pfDecodeBannerTerm`: NU encodes Fall under the *next* calendar year — Fall 2026 = `202710`; `30`=Spring, `40/50/60`=Summer). **The decode is display-only and not yet certified** as the program's first-offered/launch term: on a 30-program sample, `...10` (Fall) codes matched `eff_cat` but all `...30` codes landed one catalog year off — unresolved (Banner-season ambiguity vs. genuine eff_term/eff_cat divergence). Until that's pinned down with ground-truth launch terms, the "SVT launch ≠ CIM catalog" view still compares against `eff_cat` (catalog year), not `eff_term`. Existing rows were backfilled via `cim_http` (1826/1892 set). Migration adds the columns in `init_portfolio_tables`; on a running server they must be present before the next scan's `replace_all_portfolio_programs`, so restart the launchd server after deploying ingest changes.
