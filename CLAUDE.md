...
- **Persistence:** personal views in `localStorage['cim-portfolio-views-v1']`; **team views** shared via `GET/POST /api/portfolio/views` → `data/portfolio_views.json` (client sends its full list; server replaces). View state = `{visibleCols|gtmOnly, filters (top-bar snapshot), tree}`.
