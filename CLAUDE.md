...
- **Reconciliation: which source wins.** CIM has two views of a program's workflow state. Both can be wrong; both can disagree. We've burned hours flip-flopping which to trust, so this section is now the single decision record. **Don't re-derive this from scratch — read it, then update it if you need to change the policy.**
  ...
  **Policy: Approve Pages role discovery is authoritative; the workflow div + approval log are the fallback.** Specifically:
  ...
  | Approve Pages didn't see the program + workflow div fetched OK with steps | Walk the approval log on `/programadmin/{id}/` chronologically. Each "Approved for X" advances past the matching step at-or-after the current index; each "Rollback to X" rewinds to it. **Implicit rollback handling**: CIM sometimes records a rollback not as "Rollback to X" but as "Approved for X" where X is an earlier step. The walk handles this: if no forward match is found (all occurrences of X are before current_idx), it searches backward for the highest-order X before current_idx and sets current_idx = X.order + 1 — meaning the rollback completed and the program is now at the step immediately after where it was rolled back to. Final position is `current_step`. Walking (vs. just taking class="current") handles duplicate step names (e.g., "Program Review 2" appearing at step 3 *and* step 8 in program 177's workflow). **Parallel-branch override**: in `force_fetch_only` mode (quick role updates that skip A1 discovery), if the DB's existing `current_step` is *not* one of the workflow div's listed step names, treat it as a parallel-branch assignment (e.g., Program Graduate Provost Review on a Vancouver program whose linear workflow goes through GRA Regulatory steps) and preserve it. The walk only sees the linear branch, so without this rule each quick update silently clobbers the parallel-branch role with the linear truth. This preservation is intentionally **not** applied in full scans — A1 had the chance to see the program at any pipeline role and didn't, so the walk's result is trustworthy then. |
...
- `_lookup_cim()` has a deployment-suffix fallback: if degree has a hyphen (e.g., "MS-Align") and the exact key lookup fails, it retries with the base degree ("MS"). This handles CIM programs that store the deployment differently (e.g., "Applied AI, MPS (Arlington)" has no deployment in the degree, while "Data Science, MS—Align (Boston)" does). The first try matches the deployment-in-degree case; the fallback catches the no-deployment case.
