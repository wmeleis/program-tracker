...
### Dashboard UI
- **Source-data status bar:** an **always-visible** thin bar at the top of every view (`renderSourceHealthBanner` in `static/app.js`, driven by `GET /api/source_health` in `app.py`). Neutral (slate) by default, showing **"Data last refreshed: … · App build: …"** — `last_refresh` = latest `scans.scan_time`; `build_time` = git HEAD commit time (`_compute_build_time()` in `app.py`, falls back to newest core-file mtime). It turns **amber** and appends "Out of date (>N days): …" when any upstream source's last SUCCESSFUL refresh is older than `STALE_SOURCE_DAYS` (default **3**). Not dismissible — it's a status line, not a nag. Sources checked: CIM scan (`scans.scan_time`), SVT / GTM / GLS (from `data/portfolio_feeds/feed_health.json` `last_success`), and Regulatory (`data/last_regulatory_fetch` mtime). **OTP and IPD are intentionally excluded** — both are retired: OTP is being deprecated, and IPD's fetch was disabled 2026-05-22 (SVT is the single source of program-status truth; its ingest overlay is off), so IPD's feed file is stale by design and must not raise a false alarm. A single transient fetch failure does NOT trigger it — only genuine staleness (no success in 3 days) does. The banner fetches `/api/source_health`; on the static GitHub Pages site that fetch fails and no banner renders (it's a local-app feature). Takes effect after a Flask restart (new endpoint + cache-busted `app.js`/`style.css`).
