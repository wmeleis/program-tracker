...
**Pipeline perspective — University vs College vs EM (`cimPerspective`, CIM Programs & Courses).** A toggle in the scoping bar (above the pipeline) switches between three lenses; the choice **and** the selected college persist per browser (`localStorage['cim-perspective']` / `['cim-college-selected']` — there are no accounts, so "per user" = per browser). Note the internal value for the University lens is still `'otp'` (only the button label is "University").
...
- **EM** (Enrollment Management, value `'em'`, **graduate programs only**, university-wide): `renderEMPipeline` shows five **overlapping** filter tiles — **College / UGCC / Registrar / Ready for GTM / Inactivations**. Scope (`getBaseFiltered` + `emIncluded`): graduate programs that are either a **new offering** (`new_offering` set — new_concentration/new_degree/new_cert, from CIM XML) or an **inactivation** (`inactivates='Yes'`); plain edits are excluded; completed programs stay visible. `status` is NOT used — it can't distinguish new offerings (see "New/removed offerings" under Program Classification). **Ready for GTM** (`emReadyForGTM`) applies only to new offerings whose governance has cleared: new concentration/cert past the **Graduate** Curriculum Committee (`EM_UGCC_STEP`, ord 6), new degree past the **Board of Trustees** (ord 11), or completed — inactivations are never "GTM". Tiles overlap (a program can be both "Registrar" and "Ready for GTM"); they're quick filters, not a partition. Helpers: `EM_STAGE_ORD` (College=0 … Teach-Out=13), `emStageOrd`, `emIsNewOffering`/`emIsInactivation`/`emReadyForGTM`, `emBucketMatch(p, '__em_college__'|'__em_ugcc__'|'__em_registrar__'|'__em_gtm__'|'__em_inactivations__')`. Registrar tile = canonical "Program Setup"; Ready-for-GTM tile green (`.em-gtm`), Inactivations red (`.em-inact`). `syncPerspectiveUI` hides the level switch + college picker for EM. No courses; reads `new_offering`/`inactivates`/`current_step`/`completion_date` (all in `data.json` via `get_all_programs`). Requires a backfill rescan to populate the CIM fields.
...
All three are graduate-only (gated on `programs.program_type='Graduate'`), stored on `portfolio_programs`, and exported via `get_all_portfolio_programs()` (`SELECT p.*`). Surfaced as the `defaultHidden` **"New Offering"** column (`offering`, label via `portfolioOfferingLabel`) and three filter-tree fields: `offering` (select), `ready_gtm` (boolean), `gtm_inact` (boolean). Two permanent **system views** (defined as `GTM_VIEW` / `GTM_NEEDS_ACTION_VIEW` consts in `static/app.js`, always present as sidebar entries + top tiles in both Flask and static): **"GTM"** = `ready_for_gtm OR gtm_inactivation` (everything EM should be aware of, ~191), and **"GTM — Needs Action"** = that set AND `gtm_type` empty (no GTM Roster action on file yet, ~164). The GTM-needs-action gap is the whole point — it identifies new offerings/inactivations EM hasn't started GTM work on. Migration adds the three columns in `init_portfolio_tables`; on a running server they must be present before the next scan's `replace_all_portfolio_programs`, so restart the launchd server after deploying ingest changes.
