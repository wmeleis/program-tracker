...
- **Source-data freshness UI** (two separate pieces, `renderSourceHealthBanner` in `static/app.js`, driven by `GET /api/source_health` in `app.py`):
  1. **Always-visible freshness line** (`#freshness-row`, directly below the header) — shown on **both** the local app AND the deployed GitHub Pages site (everyone sees it): `#last-updated` shows **"Updated: … ET"** (last `scans.scan_time`) and the adjacent `#app-build` shows **"Build: … ET"** (git HEAD commit time). On the local app these come from `/api/scan/status` + `/api/source_health`; on the static site they're **baked into the export** — `export_data()` writes `build_time` (via `_build_time()` in `export_static.py`) and `last_scan`, and `build_static_js`'s dashboard overrides set both spans from that data. (Build time via `_compute_build_time()` in `app.py` / `_build_time()` in `export_static.py`, git HEAD commit, falling back to newest core-file mtime.)
...
- **Admin gate:** `_pvIsAdmin()` = `!window._staticMode` — running on the local Flask app *is* the admin credential (no URL flag); the public GitHub Pages site is never admin. On local the Views button shows the ADMIN pill and team views are fully editable; on the static site team views are read-only. The Views button always reads `★ Views` and shows a purple **ADMIN** pill when admin; the modal sidebar's Team section shows the same pill. Only admins can create/edit/reorder/delete **team** views — non-admins see only `☆ Star · Save as My View · Cancel · Apply` on team views and no `Save as Team View`. Personal views are always fully editable by their owner. The Views button label never changes to the active view name — the active view is shown by the highlighted tile in the VIEWS band.
...
Curriculum-extracted concentrations are stored in `concentrations_json` as `[{name, college, status, svt_status}]` objects. The college is captured during extraction in `_extract_concentrations_from_html()` from headings like `"Foo Concentration — Coll of X"` or `"Foo Concentration - Coll of X"` (em-dash OR hyphen+space), normalized via `_normalize_college()` (which strips diacritics, handles `Health Sciences` ↔ `Hlth Sciences` and `Science` ↔ `Sciences` variants, and runs lookup through `_COLLEGE_ALIASES`). The extractor also recognizes a concentration **menu** `<ul>` sitting under a plain `"<h2>Concentrations</h2>"` heading (not only the literal phrase "Concentration Options") — this rescues concentrations whose own section heading omits the word "Concentration" (e.g. AI MS's "Human-AI Collaboration Systems"). If a heading carries no college attribution, the field is left blank and the renderer falls back to the parent's college at display time.
