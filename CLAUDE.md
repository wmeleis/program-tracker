...
- **EM** (Enrollment Management, value `'em'`, **programs only**, university-wide): `renderEMPipeline` shows four **overlapping** filter tiles — **College / UGCC / Registrar / Ready for GTM** — instead of the full pipeline. Scope: `getBaseFiltered` restricts EM to new offerings + inactivations (`emIncluded` = `status` in {`Added`, `Deactivated`}; plain `Edited` revisions are ignored), and completed programs stay visible (they're "Ready for GTM"). **Ready for GTM** (`emReadyForGTM`) = governance cleared: concentrations/certs past the **Graduate** Curriculum Committee (the grad-focused "UGCC" gate, ord 6), everything else past the **Board of Trustees** (ord 11), or completed. The tiles overlap (a program can be both "Registrar" and "Ready for GTM"); they're quick filters, not a partition — programs sitting mid-governance (Provost reviews / Faculty Senate / BOT) just appear in the default unfiltered EM table. Helpers: `EM_STAGE_ORD` (canonical-stage ordinal, College=0 … Teach-Out=13), `emStageOrd`, `emBucketMatch(p, '__em_college__'|'__em_ugcc__'|'__em_registrar__'|'__em_gtm__')`. The "Registrar" tile maps to the canonical "Program Setup" stage; the "Ready for GTM" tile is green (`.em-gtm`). `syncPerspectiveUI` hides the level switch (EM is grad-focused) and the college picker; the standalone College/Campus/Approver/Step filters still work. No courses, no static-export-specific override (EM computes from `status`/`current_step`/`completion_date`/name already in `data.json`).
