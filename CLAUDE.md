...
- **Frontend (`static/app.js`):** `buildProgramActionPanel(programId, steps)` renders an Approve / Send-back / Comment panel inside the program row's Workflow tab (programs only, only when there's a current step). `submitProgramAction()` shows a `confirm()` with exactly what will happen, posts, surfaces the result, and reloads. The panel is hidden unless the local tracker server is reachable (`ensureLocalServer()` / `localApiBase()` → `http://localhost:5001` in static mode), so on the public GitHub Pages site the controls only appear on the machine actually running the tracker with the CIM session. CIM still enforces *who* may approve.
- **Scope/limits (v1):** programs only (courses go through CIM's approval too but aren't wired yet); the panel shows on any in-workflow program and relies on CIM's permission enforcement rather than pre-filtering to roles the user owns; standalone "comment" posts the comment step only (best-effort — a comment normally rides with an approve/send-back).
