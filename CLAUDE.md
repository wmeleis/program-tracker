...
- **Staleness guard (cross-check):** `batch_fetch_program_details` returns the full approval history (`meta.approvals`). Phase 3 compares the Approve Pages-discovered `current_step` against that history: if the discovered step is already in the approved-history set, Approve Pages is stale and we defer to the per-program workflow HTML's `current` marker. Fixed the `MS Management (Boston)` / `MS Management (Online)` stuck-at-GPR bug from April 2026.
- **On-demand heal:** `POST /api/heal` runs the same role-by-role stale-row cleanup as end-of-scan validation, but verifies each candidate-to-clear by re-fetching its workflow HTML first. `scraper.heal_stale_program_steps(log=True)` is the shared implementation. ~30s per call.
