...
- **Admin gate:** `_pvIsAdmin()` = `?admin=1` in the URL **AND** not the static site (`!window._staticMode`) — admin is only possible on the local Flask app; the public GitHub Pages site can never be admin even with `?admin=1`. The Views button always reads `★ Views` and shows a purple **ADMIN** pill when admin; the modal sidebar's Team section shows the same pill. Only admins can create/edit/reorder/delete **team** views (incl. built-in All/GTM) — non-admins see only `☆ Star · Save as My View · Cancel · Apply` on team views and no `Save as Team View`. Personal views are always fully editable by their owner. The Views button label never changes to the active view name — the active view is shown by the highlighted tile in the VIEWS band.
