...
- **College**: pick a college → the pipeline bar becomes *that college's* internal roles as fine-grained tiles (`renderCollegePipeline`), ordered by a role-type template (`collegeRoleRank`: Director → Chair → Program Review → Curriculum Committee → Dean's Office → Accreditor), with short labels (`collegeRoleShort`). The bar is a **two-panel slide** driven by a directional arrow that *replaces the "Pipeline Summary" heading itself* in college perspective (`#pipeline-panel-toggle`, styled like the h2; the static h2 is hidden). Its label flips between "College Pipeline Summary ▸" (showing college roles) and "◂ University Pipeline Summary" (showing the FULL university pipeline stages, all shown, scoped to the college, for progress/timing) — conveying one continuous pipeline, and living in the header so it never scrolls off the horizontally scrollable bar. `collegePanel` state, `setCollegePanel`; the bar slide-animates on each swap. (University perspective keeps the static "Pipeline Summary" h2.) The arrow also shows the **hidden panel's count** (`.panel-arrow-alt`, e.g. "University pipeline (14) ▸") so totals reconcile at a glance: college-tile sum + that count = the in-scope total (= the level button / list count). Without it a college whose programs have all advanced to university stages (e.g. Khoury, all 14 grad fold to "Program Setup") shows an empty College panel that looks broken vs. the button/list. `getBaseFiltered` scopes everything to the selected college (`item.college`), and the standalone College filter is hidden to avoid a conflicting double-filter. Roles carrying a *different* college's code prefix (cross-listed routing) are excluded from a college's tiles (`_COLLEGE_CODE`). Works for both programs and courses. **Tile set:** the College pipeline shows a college's FULL internal role sequence, not just currently-occupied roles. It unions (a) any role a program is *currently* at (real position, from `current_step`) with (b) the college's own roles drawn from the workflow *definitions* — `database.get_workflow_role_pairs()` returns distinct `(college, step_name)` pairs (programs + courses), exposed at `/api/workflow_roles` and baked into `data.json` as `workflow_roles`; the frontend (`cimRolePairs`, `ensureCimRolePairs`) classifies them. Because the raw workflow union is noisy (cross-listed programs route through OTHER colleges' committees; ad-hoc reviewer / checkpoint steps), the empty-role additions are **restricted to roles carrying the college's own code prefix** (`_COLLEGE_CODE[fullName]`, e.g. `Program BV …`). Currently-occupied roles are always kept regardless of prefix. Department-level chairs without the college prefix (e.g. `Program PHSC Chair`, `Program MSCI Accreditor Approval`) are shown only when currently occupied. The role *set* is derived live; only the *ordering* is templated — no per-college role lists to maintain.
