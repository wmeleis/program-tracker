...
**Exit master's flag (`exit_masters` field):** A curated set of banner codes (`EXIT_MASTERS_BANNERS` in `portfolio_ingest.py` — MS-POPU, MS-APNR, MS-BIOL, MS-MRES, MS-PSYC, MA-SOCI) flags designated "exit master's" programs. The ingest overlay (right after the GTM overlay) resolves those banner codes to CIM program ids via the `programs` table, then sets `portfolio_programs.exit_masters = 'Yes'` on matching rows and `'No'` on all others. Surfaced as the `defaultHidden` **"Exit Master's"** column (key `exitmasters`) and the `exit_masters` filter field. In `compute_db_fingerprint()`. To change the set, edit `EXIT_MASTERS_BANNERS` (uppercase banner codes).
