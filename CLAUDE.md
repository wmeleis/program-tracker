...
- **Admin gate:** `_pvIsAdmin()` = `!window._staticMode` — the local Flask app (dean's machine) can save/reorder/delete Team views; the static site is read-only for team views (personal views still editable in the visitor's own localStorage).
