...
- **References** (`fetch_reference_curricula_http`): same C3 targeted-fetch semantics, now via `fetch_reference_version` over HTTP. Per-scan targeting = **in-workflow** programs missing a ref + programs just completed (+ Boston-counterpart deps); the historical missing-ref backlog is left to the weekly full refresh rather than retried fruitlessly every scan. A reference is a program's *last-approved* version, which can only change on a completion — and every completion path (real-time `completed_in_scan`, the HTTP scan's gone-path detection, Boston-counterpart completions, and the weekly sweep's `new_completion_ids`) feeds the targeted set, so targeting completions is exact, not lossy. **Weekly safety net:** when the weekly sweep runs (`weekly_ref_refresh` flag in `do_scan`), `targeted_ids` is widened to *all* programs for one full re-fetch; the `version_id` comparison means only genuinely-advanced refs get written, so it's cheap. Uploaded/custom references (`programs.custom_reference_id` → `custom_references` table) are a separate override that `upsert_program` never touches and that always wins over the auto reference. Curriculum body extracted in pure Python (`_extract_div_inner` balances nested `<div>`s for `bodycontentframediv3` + `concentrations` + `overviewcontentframediv4`).
