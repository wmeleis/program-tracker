...
### Dashboard UI
- **Source-data staleness banner:** amber, dismissible banner at the top of every view (`renderSourceHealthBanner` in `static/app.js`, driven by `GET /api/source_health` in `app.py`). Warns when any upstream source's last SUCCESSFUL refresh is older than `STALE_SOURCE_DAYS` (default **3**). Sources checked: CIM scan (`scans.scan_time`), SVT / IPD / GTM / GLS (from `data/portfolio_feeds/feed_health.json` `last_success`, with IPD falling back to `portfolio_smartsheet.tsv` mtime), and Regulatory (`data/last_regulatory_fetch` mtime). **OTP is intentionally excluded** (being deprecated). A single transient fetch failure does NOT trigger it — only genuine staleness (no success in 3 days) does. The banner fetches `/api/source_health`; on the static GitHub Pages site that fetch fails and no banner renders (it's a local-app feature). Takes effect after a Flask restart (new endpoint + cache-busted `app.js`/`style.css`).
