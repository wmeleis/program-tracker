...
def fetch_banner_pmc_tableau():
    """Download the Registrar's Banner "Program, Major, Concentration" view.

    Authoritative source for each concentration's managing college. Same REST +
    PAT flow, signed into the Registrar site. Refreshed DAILY (Banner is the
    live system of record) via data/last_banner_pmc_fetch. Saves
    banner_program_major_concentration.csv.
    """
    import requests as _requests

    print("\n--- Banner Program/Major/Concentration (Tableau) ---")

    stamp = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'last_banner_pmc_fetch')
    if os.path.exists(stamp):
        age_h = (time.time() - os.path.getmtime(stamp)) / 3600.0
        if age_h < 24:
            print(f"  Skipping (last fetch {age_h:.1f}h ago, < 24h)")
            return
    ...
    api_ver = '3.24'
    site_name = 'Registrar'
    view_url_name = 'ProgramMajorConcentration'
    ...
    if status != 200:
        print(f"  Tableau sign-in failed (HTTP {status}): {raw[:200]}")
        _record_feed_health('Banner PMC (Tableau)', False, f'sign-in HTTP {status}')
        return
    signin_data = json.loads(raw)
    auth_token = signin_data['credentials']['token']
    site_id    = signin_data['credentials']['site']['id']
    ...
    try:
        ...
        if not views:
            print(f"  View '{view_url_name}' not found (HTTP {status}) — skipping")
            _record_feed_health('Banner PMC (Tableau)', False, 'view not found')
            return
        view_id = views[0]['id']
        print(f"  Found view: {views[0].get('name')} (id={view_id[:8]}…)")

        data_url = f"{server}/api/{api_ver}/sites/{site_id}/views/{view_id}/data"
        status, raw = _req('GET', data_url, token=auth_token, accept='*/*', timeout=180)
        if status != 200:
            print(f"  CSV download failed (HTTP {status}): {raw[:200]}")
            _record_feed_health('Banner PMC (Tableau)', False, f'CSV download HTTP {status}')
            return
        csv_text = raw.decode('utf-8-sig')
        out = os.path.join(OUTPUT_DIR, "banner_program_major_concentration.csv")
        if _save_validated('Banner PMC (Tableau)', out, csv_text, 'csv'):
            with open(stamp, 'w') as f:
                f.write(time.strftime('%Y-%m-%d %H:%M:%S'))
    finally:
        _req('POST', f"{server}/api/{api_ver}/auth/signout", token=auth_token)
...
if __name__ == "__main__":
    ...
    fetch_banner_pmc_tableau()  # Banner Program/Major/Concentration (daily)
    print("\nDone.")
