...
function loadCampusesDetail(programId) {
    ...
    const body = campuses.map(c => {
        const s = byCampus[c];
        const status = s.inact ? (s.step ? `Inactivating — ${escapeHtml(s.step)}` : 'Inactivated')
                     : s.step ? `In workflow — ${escapeHtml(s.step)}`
    ...
    contentEl.innerHTML = `
        ...
        + `<strong>${escapeHtml(base)}</strong> <span class="pa-step">(inactivations flagged)</span></div>
...
function renderPortfolioTable() {
    ...
    const emitDeploymentRows = (p, extra) => {
        ...
        const portfolioConcs = (allConcsByParent[p.id] || [])
    ...
    initTrackerShell({
        ...
        views: {
            ...
            list: () => (window._portfolioTileViews || []).map(v => ({
                id: v.id, name: v.name,
                active: v.id === portfolioActiveViewId,
            })),
            ...
            // No "All Programs" view: '' / 'all' are the cleared state → blank badge.
            activeSummary: () => {
            ...
            onClear: () => { applyPortfolioView('all'); renderPortfolioTable(); },
        },
        filters: {
