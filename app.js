...
const PORTFOLIO_REVIEW_VIEW = {
    ...
    tip: 'Portfolio review: Boston master\'s programs, sorted by Total master\'s enrollment 2026 ascending (lowest first). Shows Total master\'s enrollment (2025 & 2026), CIM status, and all OTP fields (effective + program- and deployment-grain Disposition & Notes).',
    state: {
        visibleCols: _PORTFOLIO_REVIEW_COLS,
        sortKey: 'enr_total_2026', sortDir: 1,   // Total 2026 enrollment, ascending (lowest first)
        filters: {},
        // Boston master's only.
...
function openProgramRecord(banner, defaultTab) {
    ...
    openRecordDrawer('prog_' + banner, title, ['summary'], 'summary');
}
function openPortfolioRecord(pid, cimId, hasReg, defaultTab) {
    ...
    // Summary is the single view+edit surface (identity + fields + editable OTP);
    // the alignment/curriculum tabs follow it. No standalone OTP tabs anymore.
    const tabKeys = ['summary', 'curriculum', 'reference', 'academic'];
