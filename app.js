...
function switchView(view) {
    ...
    if (view === 'portfolio') {
    ...
    } else if (view === 'portfolio_courses') {
        // Portfolio Courses (catalog browse) — self-contained filter bar + table,
        // rendered into #programs-table-container. Hide every program/portfolio
        // section; the courses view brings its own controls.
        ...
        if (filtersSection)      filtersSection.style.display = 'none';
        if (subjectGroup)        subjectGroup.style.display = 'none';
        const _vt = document.getElementById('portfolio-view-tiles'); if (_vt) _vt.style.display = 'none';
    } else {
        if (lastUpdatedEl) lastUpdatedEl.style.display = '';
    ...
    if (searchEl) {
        ...
        else if (view === 'portfolio_courses') searchEl.placeholder = 'Search catalog courses by code or title…';
        ...
        // Reset the box when entering the Courses browse (it drives _catFilters.q).
        if (view === 'portfolio_courses') { searchEl.value = _catFilters.q || ''; }
    }
    ...
    // Courses (catalog browse) has its own inline filter bar — no CIM filter toggle.
    if (cimToggle) cimToggle.style.display = (view === 'portfolio' || view === 'portfolio_courses') ? 'none' : 'block';
...
const COURSE_BUCKETS = [
    ...
    {
        role: 'PR Grad Dean',
        short_name: 'PR Grad Dean',
        // The UNIVERSITY Graduate Dean's Office review (PR = Office of the Provost)
        // — its own stage, like on the program pipeline. College-specific dean's
        // offices ("AM/BA/PS/SH/EN/CS Graduate Dean's Office") stay in College.
        match: step => step === "PR Graduate Dean's Office",
    },
    {
        role: 'OTP',
        short_name: 'OTP',
...
function renderSourceHealthBanner(data) {
    ...
    bar.className = 'app-banner app-banner-warn';   // shared banner look (amber)
    const items = stale.map(s => `${escapeHtml(s.name)} (${_fmtStaleAge(s.age_hours)})`).join(', ');
    bar.innerHTML = `<span class="banner-icon">⚠</span>`
        + `<span class="banner-msg">Source data out of date — no refresh in over ${data.threshold_days} days: `
...
async function applyFilters() {
    // Portfolio Courses browse: the header search box drives the catalog course
    // filter, not the program/portfolio tables.
    if (currentView === 'portfolio_courses') {
        const _s = document.getElementById('filter-search');
        _catFilters.q = _s ? _s.value : '';
        _renderCatBody();
        return;
    }
...
function loadCampusesDetail(programId) {
    ...
    // Group matching records by campus label. Inactivations are INCLUDED (an
    // in-workflow inactivation means the campus is still actively offered — being
    // retired, not yet gone) and flagged, rather than hidden — hiding them made a
    // live deployment invisible (e.g. an Arts Admin cert being inactivated in
    // Boston vanished from the list while still being the reference).
    const byCampus = {};   // campusLabel -> {step, completed, inact}
    (allPrograms || []).forEach(p => {
        ...
        const slot = byCampus[campus] || (byCampus[campus] = {step: '', completed: false, inact: false});
        const isInact = (p.status || '') === 'Deactivated';
        if (isInact) slot.inact = true;
    ...
    // Status → label + color. Colored pills (plus a count-by-status breakdown and
    // a big total) make the campus footprint scannable at a glance instead of a
    // flat white list.
    const COLORS = {
        green: {fg: '#166534', bg: '#dcfce7', dot: '#16a34a'},
        blue:  {fg: '#1e40af', bg: '#dbeafe', dot: '#2563eb'},
        amber: {fg: '#92400e', bg: '#fef3c7', dot: '#d97706'},
        red:   {fg: '#991b1b', bg: '#fee2e2', dot: '#dc2626'},
        grey:  {fg: '#475569', bg: '#f1f5f9', dot: '#94a3b8'},
    };
    function statusInfo(s) {
        if (s.inact && s.step) return {label: 'Inactivating', detail: s.step, cls: 'amber'};
        if (s.inact)           return {label: 'Inactivated',  detail: '',     cls: 'red'};
        if (s.step)            return {label: 'In workflow',  detail: s.step, cls: 'blue'};
        if (s.completed)       return {label: 'Approved',     detail: '',     cls: 'green'};
        return {label: 'Not tracked', detail: '', cls: 'grey'};
    }
    const tally = {};
    const rows = campuses.map(c => {
        const s = byCampus[c];
        const si = statusInfo(s);
        tally[si.cls] = (tally[si.cls] || 0) + 1;
        const col = COLORS[si.cls];
        const isThis = parseCampusFromName(prog.name).campus
            ? (parseCampusFromName(prog.name).campus === c)
            : (c === 'Boston');
        const pill = `<span style="display:inline-block;padding:2px 9px;border-radius:11px;`
            + `font-size:12px;font-weight:600;color:${col.fg};background:${col.bg}">${escapeHtml(si.label)}</span>`
            + (si.detail ? ` <span style="color:#64748b;font-size:12px">${escapeHtml(si.detail)}</span>` : '');
        return `<tr style="background:${isThis ? '#eff6ff' : '#fff'};border-top:1px solid #eef2f7">`
            + `<td style="padding:8px 12px;font-weight:${isThis ? '600' : '500'};color:#0f172a">`
            + `${escapeHtml(c)}${isThis ? ' <span class="campus-current">(this record)</span>' : ''}</td>`
            + `<td style="padding:8px 12px">${pill}</td></tr>`;
    }).join('');

    const order = [['green', 'approved'], ['blue', 'in workflow'],
                   ['amber', 'inactivating'], ['red', 'inactivated'], ['grey', 'not tracked']];
    const breakdown = order.filter(([k]) => tally[k]).map(([k, lbl]) => {
        const col = COLORS[k];
        return `<span style="display:inline-flex;align-items:center;gap:5px;margin-right:16px">`
            ...
            + `<span style="font-size:13px;color:#334155">${tally[k]} ${escapeHtml(lbl)}</span></span>`;
    }).join('');

    contentEl.innerHTML = `
        <div style="display:flex;align-items:baseline;gap:10px;margin-bottom:4px">
            <span style="font-size:28px;font-weight:700;color:#0f172a;line-height:1">${campuses.length}</span>
            <span style="font-size:14px;color:#475569">campus${campuses.length === 1 ? '' : 'es'} with a CIM record for <strong>${escapeHtml(base)}</strong></span>
        </div>
        <div style="margin:8px 0 12px">${breakdown}</div>
        <table class="campuses-table" style="width:100%;border-collapse:collapse;background:#fff;border:1px solid #e5e9f0;border-radius:8px;overflow:hidden">
            <thead><tr style="background:#f8fafc;text-align:left">
                <th style="padding:8px 12px;font-size:11px;color:#64748b;font-weight:600;text-transform:uppercase;letter-spacing:.04em">Campus</th>
                <th style="padding:8px 12px;font-size:11px;color:#64748b;font-weight:600;text-transform:uppercase;letter-spacing:.04em">Status</th>
            </tr></thead>
            <tbody>${rows}</tbody>
        </table>
        <div style="margin-top:8px;font-size:12px;color:#94a3b8">Inactivations are flagged, not hidden.</div>`;
}

// Linkify course codes in the code column of a rendered curriculum so each opens its
// catalog entry (stacked panel). Operates on `.codecol` cells only (the code column),
// so it never touches prose/titles; called by the Curriculum tab (local + static).
function _linkifyCourseCodes(container) {
    if (!container || typeof openCourseEntryPanel !== 'function') return;
    const re = /[A-Z]{2,6}\s\d{3,4}[A-Z]?/g;
    container.querySelectorAll('td.codecol, .codecol').forEach(cell => {
        // Normalize nbsp → space BEFORE escaping (escapeHtml turns   into
        // "&nbsp;", which would then break the code regex on the escaped string).
        const txt = (cell.textContent || '').replace(/ /g, ' ');
        re.lastIndex = 0;
        if (!re.test(txt)) return;
        cell.innerHTML = escapeHtml(txt).replace(re, m => {
            const code = m.replace(/\s+/g, ' ').trim();
            return `<a class="cc-link" onclick="event.stopPropagation();openCourseEntryPanel('${code}')">${m}</a>`;
        });
    });
}
        ...
        return `<div class="refs-list-item">
            <div class="refs-list-item-info">
                ...
                <div class="refs-ugcc-edit">
                    <label><input type="checkbox" id="ugcc-chk-${r.id}" ${approved ? 'checked' : ''} onchange="saveRefUgcc(${r.id})"> UGCC approved</label>
                    ${approved ? `<input type="date" id="ugcc-date-${r.id}" value="${escapeHtml(r.ugcc_date || '')}" onchange="saveRefUgcc(${r.id})" title="Approval date">` : ''}
                </div>
            </div>
...
async function saveRefUgcc(refId) {
    ...
    // The date field only exists once "approved" is checked; treat as blank otherwise.
    const dateEl = document.getElementById('ugcc-date-' + refId);
...
function switchDetailTab(programId, tab) {
    ...
    if (tab === 'summary') loadSummaryDetail(programId);
    else if (tab === 'workflow') loadWorkflowDetail(programId, currentView === 'courses');
...
async function loadPrecheckDetail(programId) {
    ...
    if (!el) return;
    const isCourse = (currentView === 'courses');
    el.innerHTML = '<div class="workflow-loading">Running Catalog Readiness check…</div>';
    let d;
    try {
        if (window._staticMode) {
            // Read the baked precheck.json / precheck_courses.json (from the last scan).
            const cache = isCourse ? await _ensurePrecheckCoursesCache() : await _ensurePrecheckCache();
            d = isCourse ? cache[String(programId)] : (cache[String(programId)] || {}).precheck;
            if (!d) throw new Error(`No Catalog Readiness data on file for this ${isCourse ? 'course' : 'program'}.`);
        } else {
            const resp = await fetch(`/api/${isCourse ? 'course' : 'program'}/${programId}/precheck`);
            d = await resp.json();
        }
        if (d.error) throw new Error(d.error);
    } catch (e) {
        el.innerHTML = `<p style="color:#b91c1c;padding:8px">Could not run Catalog Readiness check: ${e.message}</p>`;
        return;
    }
    ...
    if (isCourse) {
        // Courses have no AI tier — surface the full course rule set as a themed
        // review checklist (the judgment/human rules a reviewer should verify).
        const cl = d.checklist || [];
        const SV2 = {block: '#991b1b', fix: '#92400e', advisory: '#3730a3'};
        if (cl.length) {
            html += `<h4 style="margin:12px 0 6px;border-top:1px solid #eef2f7;padding-top:10px">Review checklist (${cl.length})</h4>`;
            const byTheme = {};
            cl.forEach(r => { (byTheme[r.theme || 'Other'] = byTheme[r.theme || 'Other'] || []).push(r); });
            for (const theme of Object.keys(byTheme)) {
                html += `<div style="margin:8px 0 3px;font-weight:600;font-size:12px;color:#1e293b">${escapeHtml(theme)}</div>`;
                for (const r of byTheme[theme]) {
                    const c = SV2[r.severity] || '#475569';
                    html += `<div style="font-size:12px;color:#475569;margin:0 0 4px;padding-left:8px">
                        <span style="color:#94a3b8">${r.id}</span>
                        <span style="color:${c};font-weight:600;font-size:10px"> ${(r.severity || '').toUpperCase()}</span>
                        — ${escapeHtml(r.rule || '')}</div>`;
                }
            }
        }
        html += '</div>';
    ...
    // AI review section (llm judgment rules). Locally it's evaluated on demand via
    // Claude; on the static site it shows the baked (cached) AI review, no button.
    html += `<div id="precheck-llm-${programId}" style="margin-top:12px;border-top:1px solid #eef2f7;padding-top:10px">
        <div style="display:flex;align-items:center;gap:8px">
          ...
          ${window._staticMode ? '' : `<button onclick="event.stopPropagation(); loadPrecheckLLM(${programId}, false)"
    ...
    html += '</div>';
    el.innerHTML = html;
    // On the static site, auto-render the baked AI review (if any) — no live run.
    if (window._staticMode) loadPrecheckLLM(programId, false);
}

// Lazy-load + cache the baked precheck_courses.json (static site only).
let _precheckCoursesCache = null;
async function _ensurePrecheckCoursesCache() {
    if (!_precheckCoursesCache) {
        try { _precheckCoursesCache = await (await fetch('precheck_courses.json')).json(); }
        catch (e) { _precheckCoursesCache = {}; }
    }
    return _precheckCoursesCache;
}

// Lazy-load + cache the baked precheck.json (static site only).
let _precheckCache = null;
async function _ensurePrecheckCache() {
    if (!_precheckCache) {
        try { _precheckCache = await (await fetch('precheck.json')).json(); }
        catch (e) { _precheckCache = {}; }
    }
    return _precheckCache;
}
...
async function loadPrecheckLLM(programId, force) {
    ...
    let d;
    if (window._staticMode) {
        // Static site: use the baked (cached) AI review; no live evaluation.
        const cache = await _ensurePrecheckCache();
        d = (cache[String(programId)] || {}).llm
            || {available: false, reason: 'No AI review was cached for this program at publish time.'};
    } else {
        try {
...
// A leading "or "/"and " on a course code is a CONNECTOR (an or-/and-alternative
// row) ONLY when a real course code — subject letters + a number — follows. This
// guards against a course whose SUBJECT is literally "OR" (Operations Research)
// or "AND": "OR 6205" is a standalone course, not an alternative of the row above
// it, and its code must NOT be stripped to "6205". "or OR 6205" (an alternative
// that happens to be an OR course) still strips its leading connector correctly.
const _ALT_LEAD_RE = /^(?:or|and)\s+(?=[A-Za-z]{2,6}\s*\d)/i;
function _isAltCode(code) { return _ALT_LEAD_RE.test(code || ''); }
function _stripAltLead(code) { return (code || '').replace(_ALT_LEAD_RE, ''); }
...
function extractCourseLines(html) {
    ...
    allElements.forEach(el => {
        ...
        if (isAreaHeader) {
            ...
            const normalizedCode = codecol.replace(/\u00a0/g, ' ').replace(/\s+/g, ' ').trim().replace(_ALT_LEAD_RE, '');
            const allCodes = normalizedCode.match(/[A-Z]{2,5}\s+\d{4}[A-Z]?/gi) || [];
            if (allCodes.length >= 2 && /\band\s+[A-Z]{2,5}\s+\d{4}/i.test(normalizedCode)) {
                // The title cell mirrors the code cell's "X and Y" structure (the
                // <br> in "Mathematical Concepts<br>and Python" was flattened to a
                // space by cleanCurriculumHtml), so split the title on the SAME
                // "and" connector to give each code its own title \u2014 instead of
                // dumping the whole combined title on the first code (which left the
                // second course title-less, e.g. AAI 5025 blank). Only when the
                // segment count matches the code count; otherwise fall back safely.
                let titleParts = titlecol ? titlecol.split(/\s+and\s+/i).map(s => s.trim()).filter(Boolean) : [];
                if (titleParts.length !== allCodes.length) titleParts = null;
                allCodes.forEach((code, idx) => {
                    const upper = code.toUpperCase().replace(/\s+/g, ' ');
                    // Non-first codes keep the "and " connector on their DISPLAY code
                    // (key stays the bare code for diffing) so mergeAlts groups them
                    // under the first course and the Compare view shows "AAI 5015 …
                    // and AAI 5025" as one required pair, not two disconnected rows.
                    lines.push({
                        key: upper,
                        code: idx === 0 ? upper : ('and ' + upper),
                        title: titleParts ? titleParts[idx] : (idx === 0 ? titlecol : ''),
...
function diffLines(oldLines, newLines) {
    ...
    function entrySetKey(entry) {
        ...
        for (const a of (entry.alts || [])) {
            const stripped = (a.code || '').replace(_ALT_LEAD_RE, '');
...
function _codePresent(codeStr, discreteSet, ranges) {
    const c = normForCompare((codeStr || '').replace(_ALT_LEAD_RE, ''));
...
// Wrap a course code in a clickable link to its catalog entry (stacked panel),
// but only when it's a real "SUBJ NNNN" code — leading "and/or" is stripped for the
// link target; wildcards / ranges / anything else stay plain text.
function _courseCodeLink(display) {
    const clean = (display || '').replace(_ALT_LEAD_RE, '').replace(/ /g, ' ').replace(/\s+/g, ' ').trim();
    if (!/^[A-Z]{2,6}\s\d{3,4}[A-Z]?$/.test(clean)) return escapeHtml(display);
    return `<a class="cc-link" onclick="event.stopPropagation();openCourseEntryPanel('${clean}')">${escapeHtml(display)}</a>`;
}
...
function renderCourseCell(item, cls, otherItem, mySide, oppositeAll, oppositeRanges) {
    ...
    const isAlt = _ALT_LEAD_RE.test(item.code || '');
    ...
    let codeHtml = `<span class="${p}">${isWild ? escapeHtml(item.code) : _courseCodeLink(item.code)}</span>`;
    ...
    if (item.alts && item.alts.length) {
        for (const a of item.alts) {
            ...
            codeHtml += `<div class="cmp-alt-line ${ap}">${isWild ? escapeHtml(a.code) : _courseCodeLink(a.code)}</div>`;
...
function renderSideBySide(diff, leftLabel, rightLabel) {
    ...
    const collect = (set, ranges, item) => {
        ...
        const norm = c => normForCompare((c || '').replace(_ALT_LEAD_RE, ''));
...
function compareCurricula(refHtml, currHtml) {
    ...
    function entrySet(entry) {
        ...
        const codes = [normForCompare(entry.code || '')];
        for (const a of (entry.alts || [])) {
            codes.push(normForCompare((a.code || '').replace(_ALT_LEAD_RE, '')));
        }
        return codes.filter(Boolean).sort().join('|');
    }
...
const COLLEGE_ABBREVS = {
    ...
    // Course-catalog (Banner Course Inventory) college spellings — mapped to the
    // same program-list acronyms so the Portfolio → Courses college pulldown is
    // consistent with the rest of the app.
    'Mills College at Northeastern': 'MCNU',
    'College of Arts & Sciences': 'CAS',      // legacy Banner college (pre-split)
    'School of Engineering Tech': 'SET',
    'NU Global Network': 'NUGN',
    'No College Designated': 'None',
    'All': 'UNIV',                            // Banner "All" = university-wide course
};
...
function renderConsoleContent(data) {
    ...
    if (nonPrograms.length) {
        ...
        html += `<details style="margin-top:16px"><summary style="cursor:pointer;font-size:13px;font-weight:600;color:#64748b">Not ingested — non-proposal SVT requests &amp; non-degree entries (${nonPrograms.length})</summary>`;
        html += '<p style="color:#64748b;font-size:11px;margin:4px 0 8px">These carry real program names but are deliberately not ingested: SVT rows logged as market-research or consultation requests (not proposals moving through a pipeline), or entries that aren’t a trackable degree program (boot camps, badges, course codes, multi-program bundles). The "Why" column gives the specific reason.</p>';
        for (const [src, rows] of Object.entries(bySource)) {
            ...
            html += '<thead><tr style="background:#f8fafc;text-align:left"><th style="padding:3px 8px">Name</th><th style="padding:3px 8px">Why not ingested</th></tr></thead><tbody>';
            for (const e of rows) {
                html += `<tr style="border-top:1px solid #e2e8f0"><td style="padding:3px 8px;color:#64748b">${escapeHtml(e.source_name)}</td>`
                     + `<td style="padding:3px 8px;color:#64748b">${escapeHtml(e.reason || '—')}</td></tr>`;
            }
            html += '</tbody></table>';
        }
        html += '</details>';
...
// ============ App surfaces: separate CIM / Portfolio pages ============
// App surface (?app=…) — the CIM and Portfolio functions are separate "pages" on
// the one build, selected by the query param (so the shared console can point a
// different link at each). Values:
//   'cim'        → CIM-only surface (Programs / Courses / Catalog, full drawer)
//   'portfolio'  → Portfolio-only surface (Program / Deployments / Matrix, Views)
//   ''           → combined (both nav groups) — the default admin view
function _appSurface() {
    let v = '';
    // A dedicated page path (/cim, /portfolio) sets window.__APP_SURFACE__ in the
    // served HTML; the ?app= query param overrides it (for ad-hoc links).
    try { v = (new URLSearchParams(location.search).get('app') || '').toLowerCase(); } catch (e) {}
    if (!v && typeof window.__APP_SURFACE__ === 'string') v = window.__APP_SURFACE__.toLowerCase();
    return (v === 'cim' || v === 'portfolio') ? v : '';
}
// A surface that shows only the CIM nav group.
function _isCimSurface() { return _appSurface() === 'cim'; }
function _isPortfolioSurface() { return _appSurface() === 'portfolio'; }

function _initDashboard() {
    // Which "page" is this? CIM-only, Portfolio-only, or combined.
    const _surface = _appSurface();
    // Restore last active view (so navigating away and back keeps your context).
    // A surface pins its own home view and does NOT restore/deep-link across pages.
    let savedView = 'programs';
    if (!_surface) { try { savedView = localStorage.getItem('cim-active-view') || 'programs'; } catch(e) {} }
    // Deep-link (used by the shared console's "Needs You" pane + the Section
    // Tracker's section→course reverse link):
    //   ?view=<programs|courses|catalog|portfolio|portfolio_courses> overrides the view,
    ...
    //   ?search=<course code>        pre-fills the Courses browse search (view=portfolio_courses).
    let _dqStep = null, _dqPView = null, _dqSearch = null;
    try {
        const _q = new URLSearchParams(location.search);
        const qv = _q.get('view'); if (qv) savedView = qv;
        _dqStep = _q.get('step'); _dqPView = _q.get('pview'); _dqSearch = _q.get('search');
    } catch(e) {}
    const validViews = ['programs', 'courses', 'catalog', 'portfolio', 'portfolio_courses'];
    if (!validViews.includes(savedView)) savedView = 'programs';
    // Pin each surface to its own function; ?view= may still pick among that
    // surface's own views but can't cross into the other page. The Portfolio surface
    // hosts BOTH the program portfolio and the Courses browse, so allow either.
    if (_isCimSurface() && savedView === 'portfolio') savedView = 'programs';
    if (_isPortfolioSurface() && savedView !== 'portfolio_courses') savedView = 'portfolio';
    // ?search= pre-fills the Courses browse (set before its first render).
    if (_dqSearch && savedView === 'portfolio_courses') _catFilters.q = _dqSearch;
...
const PORTFOLIO_COLUMNS = [
    ...
    {key: 'bannercode',   label: 'Banner code', defaultHidden: true,
        help: 'Banner program code (shared across a program\'s campus deployments). From the CIM record; blank for external-feed-only rows or in-workflow proposals Banner has not coded yet.'},
    ...
    {key: 'otplaunchterm', label: 'OTP First Intake Term', defaultHidden: true,
        help: 'A first-intake term you set by hand in the record drawer (two-grain: a program-level term shared across campuses, or a per-deployment override; most-specific-wins). Independent of SVT — records the term you expect students to first enter. Feeds the "At risk: launches" term check (via the effective/earliest first-intake term).'},
    {key: 'otpgtmdate', label: 'OTP GTM Date', defaultHidden: true,
        help: 'A go-to-market DATE you set by hand in the record drawer (two-grain; most-specific-wins). A separate at-risk signal — it does NOT change the ingested GTM Roster date shown in the "GTM Date" column. Feeds the "At risk: launches" GTM-date (next 120 days) check.'},
    ...
    {key: 'reqcore',     label: 'Required Core Courses', defaultHidden: true,
        help: 'Number of strictly-required courses taken by all students (no choice, not concentration-specific), from the published graduate catalog. Master’s programs only.'},
    {key: 'reqtotal',    label: 'Total Required Courses', defaultHidden: true,
        help: 'Distinct strictly-required courses including concentration-specific ones, from the published graduate catalog. Master’s programs only.'},
    {key: 'requiresst',  label: 'Requires ST Course', defaultHidden: true,
        help: 'Yes when a strictly-required course is a special-topics (variable-content) course, matched against the Section Tracker’s canonical ST code list.'},
    {key: 'reqstcount',  label: '# Required ST Courses', defaultHidden: true,
        help: 'Number of strictly-required special-topics courses in the program’s required curriculum.'},
    {key: 'reqstcourses', label: 'Required ST Courses', defaultHidden: true,
        help: 'The strictly-required special-topics courses (code and title), from the published graduate catalog.'},
...
const _PF_DEFAULT_W = {
    ...
    expectedterm: 120, otplaunchterm: 130, otpgtmdate: 120, ciminact: 110, inworkflow: 90, offering: 120, gtmentered: 120,
};
...
// The Go-to-market (enrollment-management) system views. Graduate-only signals
// derived from CIM: new offerings (concentration/degree) that cleared governance.
// Inactivations are EXCLUDED (Waleed 2026-09-18) — go-to-market is about launching;
// retirements live in the "Master's being retired (recent)" view. Columns focus on
// identity + offering + GTM roster data.
...
const _GTM_RELEVANT_GROUP = {
    type: 'group', conj: 'all', children: [
        ...
        // Exclude inactivations even when they carry a ready-for-GTM flag (some
        // inactivation records also flag a new offering) — go-to-market is launches.
        { type: 'rule', field: 'cim_change', op: 'not_in', value: ['Inactivation'] },
    ],
};
...
const GTM_VIEW = {
    id: 'gtm', name: 'Go-to-market: pipeline', team: true, system: true,
    tip: 'Graduate new offerings that have completed governance and are ready for go-to-market: a new concentration or certificate past the Graduate Curriculum Committee, a new degree past the Board of Trustees, or completed effective in the 2025-2026 catalog or later. Inactivations are excluded (see "Master\'s being retired").',
    state: {
...
const GTM_NEEDS_ACTION_VIEW = {
    id: 'gtm_needs_action', name: 'Go-to-market: no plan yet', team: true, system: true,
    tip: 'The go-to-market set (graduate new offerings ready for go-to-market) filtered to programs with no GTM Type recorded in the Go To Market Roster 2.0 — i.e. no GTM action started yet.',
    state: {
        visibleCols: _GTM_VIEW_COLS,
        filters: {},
        tree: {
...
const GTM_RECENT_VIEW = {
    id: 'gtm_recent', name: 'Go-to-market: new (last 30 days)', team: true, system: true,
    tip: 'Graduate new offerings that entered the go-to-market stage in the last 30 days — i.e. first became GTM-ready (a new offering completed governance) within the window, by GTM Entered date. Inactivations excluded. Adjust the day count in the view’s filter.',
    state: {
...
const RECENTLY_INACTIVATED_VIEW = {
    id: 'recently_inactivated', name: "Master's being retired (recent)", team: true, system: true,
...
const PORTFOLIO_REVIEW_VIEW = {
    id: 'portfolio_review', name: "Boston master's: enrollment review", team: true, system: true,
    tip: 'Portfolio review: Boston master\'s programs in the current/upcoming catalog (excluding exit master\'s, which are low-enrollment by design, and programs no longer in the catalog — retired/completed inactivations), sorted by Total master\'s enrollment 2026 ascending (lowest first). Shows Total master\'s enrollment (2025 & 2026), CIM status, and all OTP fields (effective + program- and deployment-grain Disposition & Notes).',
    state: {
        visibleCols: _PORTFOLIO_REVIEW_COLS,
        sortKey: 'enr_total_2026', sortDir: 1,   // Total 2026 enrollment, ascending (lowest first)
        filters: {},
        // Boston master's only; exclude exit master's (awarded on the way out of a
        // PhD, no direct admission — structurally low-enrollment, not review
        // candidates) and programs NO LONGER IN THE CATALOG (in_catalog = Y keeps
        // active programs AND in-workflow teach-outs still in the current catalog,
        // but drops genuinely-dead/completed inactivations like Biotechnology,
        // MS — Experiential, retired years ago).
        tree: { type: 'group', conj: 'all', children: [
            { type: 'rule', field: 'credential', op: 'in', value: ["Master's"] },
            { type: 'rule', field: 'campus', op: 'in', value: ['Boston'] },
            { type: 'rule', field: 'exit_masters', op: 'not_in', value: ['Yes'] },
            { type: 'rule', field: 'in_catalog', op: 'in', value: ['Y'] },
        ] },
    },
};

const _LAUNCH_RISK_COLS = ['degree', 'college', 'campus', 'launch', 'gtmdate', 'otpgtmdate', 'proposedterm', 'expectedterm', 'otplaunchterm', 'cim', 'cimchange', 'svt', 'svttype'];
const LAUNCH_RISK_VIEW = {
    id: 'launch_risk', name: 'At risk: launches', team: true, system: true,
    tip: 'Programs at RISK of not launching on time: EITHER a GTM date (the SVT GTM Launch date, the GTM-roster GTM Date, OR a hand-set OTP GTM date) within the next 120 days (≈4 months), OR an effective first-intake term (the soonest of your OTP first intake term and the SVT Expected/Proposed terms) within the next 2 terms — AND governance is NOT yet complete (still in the CIM approval workflow, or not in CIM at all). Programs whose governance is past — completed the workflow, OR at a post-approval build step (Setup/Banner/Editor/Catalog/Degree Audit/teach-out) — are EXCLUDED: they will launch, so they are not at risk. Inactivations are excluded too (being retired, not launched). Sorted by GTM Launch date, soonest first.',
    state: {
        visibleCols: _LAUNCH_RISK_COLS,
        sortKey: 'launch', sortDir: 1,   // soonest GTM date first
        filters: {},
        // (GTM Launch OR GTM Date within 4 months, OR effective launch term within 3
        // terms) AND governance-not-done. `past_gov='N'` means the program has NOT
        // finished governance and isn't at a post-approval build step — i.e. still at
        // risk. It is 'N' for programs still in early workflow AND for programs not in
        // CIM at all; 'Y' for completed OR build-step programs (those will launch, so
        // they're excluded). This is stricter than "cim_step is_set", which wrongly
        // kept build-step programs whose governance is already done.
        tree: { type: 'group', conj: 'all', children: [
            { type: 'group', conj: 'any', children: [
                { type: 'rule', field: 'launch',   op: 'next_days', value: '120' },
                { type: 'rule', field: 'gtm_date', op: 'next_days', value: '120' },
                { type: 'rule', field: 'otp_gtm_date', op: 'next_days', value: '120' },
                { type: 'rule', field: 'eff_launch_term', op: 'within_terms', value: '2' },
            ] },
            { type: 'rule', field: 'past_gov', op: 'in', value: ['N'] },
            // Exclude inactivations — they're being RETIRED, not launched (their
            // GTM-roster date is an inactivation date, not a launch date).
            { type: 'rule', field: 'cim_change', op: 'not_in', value: ['Inactivation'] },
        ] },
    },
};
...
const ADMIN_LAUNCH_TIMING_VIEW = {
    id: 'admin_launch_timing', name: 'Data check: launch timing', team: true, system: true, admin: true,
...
const ADMIN_SVT_COORD_VIEW = {
    id: 'admin_svt_coord', name: 'Data check: SVT with no CIM match', team: true, system: true, admin: true,
...
function getAllPortfolioViews() {
    const base = [ALL_PROGRAMS_VIEW, GTM_VIEW, GTM_NEEDS_ACTION_VIEW, GTM_RECENT_VIEW, RECENTLY_INACTIVATED_VIEW,
            PORTFOLIO_REVIEW_VIEW, LAUNCH_RISK_VIEW,
...
// `offering` / `svt_coord` / `in_cim` / `svtnote` / `credential` are all derived
// once at ingest (portfolio_fields) and read from the stored column — the old
// portfolioOfferingLabel()/_svtNeedsCoord/_svtCoordNote/_svtSubjectWords/
// _svtCimSubjects/_svtDegKey/extractPortfolioDegree helpers were removed.
...
// Linear term index (…, Fall 2026, Winter 2027, Spring 2027, …) so we can count
// how many academic terms apart two terms are (rank's year*10+season isn't
// linear across years). = year*4 + season.
function _pfTermLinear(rank) {
    if (rank == null) return null;
    return Math.floor(rank / 10) * 4 + (rank % 10);
}
...
// Effective launch term = the EARLIEST (soonest) of the human-set OTP launch
// term and the two SVT launch terms (Expected, Proposed). Whichever comes first
// is the term we'd have to be ready for. Returns a canonical term LABEL
// ("Fall 2027") or '' when none of the three parse.
function _pfEffLaunchTerm(p) {
    let best = null;
    [p.otp_launch_term, p.roster_expected_launch_term, p.roster_proposed_launch_term]
        .forEach(v => { const r = _pfTermRank(v); if (r != null && (best == null || r < best)) best = r; });
    return best == null ? '' : _pfTermLabel(best);
}
...
// `recent_inact` is now derived at ingest (portfolio_fields.recent_inact) and
// read from the stored column; _pfCatalogStartYear / _pfCurrentCatalogStartYear
// / _pfRecentlyInactivated were removed.
...
// `launch_vs_cim` is now derived at ingest (portfolio_fields.launch_vs_cim) and
// read from the stored column; the helper was removed.
...
const PORTFOLIO_FILTER_FIELDS = [
    ...
    {key: 'credential',  label: 'Credential',       type: 'select', value: p => p.credential || ''},
    ...
    {key: 'in_cim',      label: 'In CIM',           type: 'boolean', value: p => p.in_cim || 'N'},
    ...
    {key: 'recent_inact', label: 'Recently inactivated (in workflow or last 3 catalog yrs)', type: 'boolean', value: p => p.recent_inact || 'N',
    ...
    {key: 'otp_launch_term', label: 'OTP First Intake Term', type: 'term', value: p => p.otp_launch_term || '',
        help: 'A first-intake term you set by hand in the drawer (two-grain: program or deployment, most-specific-wins). Independent of SVT — use it to record the term you expect students to first enter when SVT has no date, or to override it.'},
    {key: 'eff_launch_term', label: 'Effective First Intake Term (earliest set)', type: 'term', value: p => _pfEffLaunchTerm(p),
        help: 'The soonest of the OTP first intake term, the SVT Expected term, and the SVT Proposed term — the term you would need to be ready for. Powers the "At risk: launches" term check.'},
    ...
    {key: 'launch_vs_cim',  label: 'SVT launch ≠ CIM catalog', type: 'boolean', value: p => p.launch_vs_cim || 'N'},
    {key: 'svt_coord',   label: 'Needs SVT coordination', type: 'boolean', value: p => p.svt_coord || 'N'},
    {key: 'svtnote',     label: 'SVT coordination note', type: 'text',    value: p => p.svtnote || ''},
    ...
    {key: 'cim_inact',   label: 'Inactivation in Progress', type: 'boolean', value: p => p.cim_inact || 'N',
    ...
    {key: 'admit_today', label: 'Admitting Today',  type: 'boolean', value: p => { const v = p.admit_today || ''; return v === 'Yes' ? 'Y' : v === 'No' ? 'N' : ''; }},
    {key: 'offering',    label: 'New Offering',     type: 'select', value: p => p.offering || ''},
    ...
    {key: 'gtm_date',    label: 'GTM Date',         type: 'date',   value: p => p.gtm_date || ''},   // GTM-roster date (ISO); date ops incl. next_days
    {key: 'otp_gtm_date', label: 'OTP GTM date',    type: 'date',   value: p => p.otp_gtm_date || '',
        help: 'A go-to-market DATE you set by hand in the drawer (two-grain: program or deployment, most-specific-wins). A separate signal — it does NOT change the ingested GTM Roster date. Feeds the "At risk: launches" GTM-date check, so a program with no feed GTM date (e.g. an SVT-only row not in CIM) can still be flagged.'},
    ...
    {key: 'requires_st', label: 'Requires ST Course', type: 'boolean',
        value: p => p.requires_st === 'Yes' ? 'Y' : (p.requires_st === 'No' ? 'N' : '')},
    {key: 'req_core',    label: 'Required Core Courses', type: 'select',
        value: p => (p.req_core_courses === '' || p.req_core_courses == null) ? '' : String(p.req_core_courses)},
    {key: 'req_total',   label: 'Total Required Courses', type: 'select',
        value: p => (p.req_total_courses === '' || p.req_total_courses == null) ? '' : String(p.req_total_courses)},
    {key: 'req_st_count',label: '# Required ST Courses', type: 'select',
        value: p => (p.req_st_count === '' || p.req_st_count == null) ? '' : String(p.req_st_count)},
...
function evalPortfolioRule(p, rule) {
    ...
    if (f.type === 'term') {
        ...
        if (op === 'within_terms') {
            // Forward window in academic TERMS: the term is between the current
            // term and current + N terms (inclusive). N=0 → this term only.
            if (rank == null) return false;
            const n = parseInt(rule.value, 10);
            if (isNaN(n) || n < 0) return true;   // no window set yet → don't restrict
            const now = _pfTermLinear(_pfCurrentTermRank());
            const t = _pfTermLinear(rank);
            return t >= now && t <= now + n;
        }
    ...
    if (f.type === 'date') {
        ...
        if (op === 'next_days') {
            // Forward window: date is between today and today + n days (inclusive).
            const n = parseInt(rule.value, 10);
            if (!(n > 0)) return true;            // no window set yet → don't restrict
            const today = new Date(); today.setHours(0, 0, 0, 0);
            const end = new Date(today); end.setDate(end.getDate() + n);
            const dv = new Date(d + 'T00:00:00');
            return !isNaN(dv) && dv >= today && dv <= end;
        }
...
function _opsForPvType(t) {
    ...
    if (t === 'date')    return [['within_days','in the last … days'],['next_days','in the next … days'],['on_after','on or after'],['on_before','on or before'],['is_set','is set'],['is_empty','is not set']];
    if (t === 'term')    return [
        ...
        ['within_terms','is within the next … terms'],
...
function _renderPvRuleValue(rule, f, path) {
    ...
    if (f.type === 'date') {
        if (rule.op === 'within_days' || rule.op === 'next_days') {
    ...
    if (f.type === 'term') {
        ...
        if (rule.op === 'within_terms') {
            return `<input type="number" min="0" class="pvb-text" style="width:72px" value="${escapeHtml(rule.value == null ? '' : rule.value)}"
                     oninput="pvbSetValue('${path}', this.value)" placeholder="terms"> terms`;
        }
...
function pvbSetOp(path, op)      { const w = _pvWalk(path); if (w && w.node.type === 'rule') { w.node.op = op; const t = (_pvField(w.node.field) || {}).type; if (op === 'is_set' || op === 'is_empty') w.node.value = null; else if (t === 'term') w.node.value = (op === 'before_now' || op === 'on_after_now') ? null : (op === 'within_terms') ? '3' : ''; else if (t === 'date') w.node.value = (op === 'within_days') ? '30' : (op === 'next_days') ? '120' : ''; else if (!w.node.value || (Array.isArray(w.node.value) && !w.node.value.length)) w.node.value = (t === 'text') ? '' : []; renderPvModal(); } }
...
function _programScopes() {
    ...
    return [
        ...
        // ── Portfolio → Courses (catalog browse) filters ──────────────────────
        {   // Course level BY NUMBER (Waleed: 1000-4999 = Undergrad, 5000+ = Graduate).
            kind: 'buttons', label: 'Level',
            enabled: () => currentView === 'portfolio_courses',
            options: () => [{ value: 'Undergraduate', label: 'Undergrad' }, { value: 'Graduate', label: 'Graduate' }],
            has: v => _catLevelSel.has(v),
            onSelect: v => { if (_catLevelSel.has(v)) _catLevelSel.delete(v); else _catLevelSel.add(v); _renderCatBody(); },
        },
        {   // Subject code — Courses (searchable; ~200 subjects).
            label: 'Subject', placeholder: 'All subjects', searchable: true,
            enabled: () => currentView === 'portfolio_courses',
            options: () => _catDistinct('subject').map(s => ({ value: s, label: s })),
            active: () => _catFilters.subject,
            onSelect: v => { _catFilters.subject = v || ''; _renderCatBody(); },
        },
        {   // College — Courses (searchable; standardized acronyms).
            label: 'College', placeholder: 'All colleges', searchable: true,
            enabled: () => currentView === 'portfolio_courses',
            options: () => _catDistinct('college').sort((a, b) => abbreviateCollege(a).localeCompare(abbreviateCollege(b))).map(c => ({ value: c, label: abbreviateCollege(c) })),
            active: () => _catFilters.college,
            onSelect: v => { _catFilters.college = v || ''; _renderCatBody(); },
        },
        {   // Status — Courses (Active default).
            label: 'Status', placeholder: 'Active',
            enabled: () => currentView === 'portfolio_courses',
            options: () => [{ value: 'Active', label: 'Active' }, { value: 'Inactive', label: 'Inactive' }, { value: 'All', label: 'All' }],
            active: () => _catFilters.status,
            onSelect: v => { _catFilters.status = v || 'Active'; _renderCatBody(); },
        },
        ...
        {   // Retired — Portfolio only. Long-retired programs (completed inactivations
            // >3 catalog years old) are hidden by default; this button reveals them.
            kind: 'buttons', label: 'Retired',
            enabled: () => currentView === 'portfolio',
            options: () => {
                const n = (allPortfolioPrograms || []).filter(_isLongRetired).length;
                return [{ value: 'show', label: n ? `Show retired (${n})` : 'Show retired' }];
            },
            has: () => portfolioShowRetired,
            onSelect: () => {
                portfolioShowRetired = !portfolioShowRetired;
                try { localStorage.setItem('cim-portfolio-show-retired', portfolioShowRetired ? '1' : ''); } catch(_) {}
                renderPortfolioTable();
                if (typeof updateClearButtons === 'function') updateClearButtons();
            },
        },
...
function renderPortfolioViewTiles() {
    ...
    const stars   = getPortfolioStarredIds();
    // No "All Programs" pseudo-view — the default state IS "no active view"
    // (unfiltered, all rows). Clearing an active view is the rail's VIEWS-badge
    // × (views.onClear); ALL_PROGRAMS_VIEW stays resolvable by id only, as the
    // cleared state applyPortfolioView('all') restores.
    const starredViews = [...getPortfolioTeamViews(), ...getPortfolioPersonalViews()]
                            .filter(v => stars.has(v.id));
    const tileViews = [GTM_VIEW, GTM_NEEDS_ACTION_VIEW, GTM_RECENT_VIEW, RECENTLY_INACTIVATED_VIEW,
        PORTFOLIO_REVIEW_VIEW, LAUNCH_RISK_VIEW,
...
// `cim_inact` (inactivation in progress) and `admit_today` are now derived once
// at ingest (portfolio_fields.cim_inact / admit_today) and read from the stored
// column; the old _cimInactivating / _inactAdmittingToday helpers were removed.
...
// The credential is now derived once at ingest (portfolio_fields.credential,
// a port of the old extractPortfolioDegree/_credentialFromCode/_CRED_SETS) and
// read from the stored p.credential column — the JS parser was removed.
...
async function loadPortfolioDashboard() {
    ...
    try {
        ...
        if (isRefresh) {
            ...
            // Start each load with NO view active (full portfolio). A saved view no
            // longer auto-restores: a silently-persisted view (e.g. Portfolio Review,
            // scoped to Boston master's) repeatedly hid rows and made searches come
            // up empty. The user picks a view per session when they want one.
            // (Waleed 2026-09-17.)
            renderPortfolioViewTiles();
            renderPortfolioTable();
        }
...
function populatePortfolioFilters() {
    ...
    const inactTodayVals = [...new Set(allPortfolioPrograms.map(p => p.admit_today || '').filter(Boolean))].sort();
...
// Row-colour filter — set by clicking a legend entry. The key matches the
// stored p.row_color (same logic as the row tint), so clicking "New (in
// workflow)" keeps only green rows, etc.
let portfolioColorFilter = '';
// Long-retired programs — completed inactivations older than 3 catalog years
// (recent_inact !== 'Y', so NOT in-workflow and NOT completed within 3 yrs) — are
// HIDDEN from the portfolio by default; the FOCUS "Show retired" button reveals
// them. Persisted per browser.
let portfolioShowRetired = (() => { try { return localStorage.getItem('cim-portfolio-show-retired') === '1'; } catch(_) { return false; } })();
const _isLongRetired = p => p.cim_change_type === 'Inactivation' && !p.cim_step && p.recent_inact !== 'Y';
// The row-tint / legend colour key is now derived once at ingest
// (portfolio_fields.row_color) and read from the stored p.row_color column —
// the old _pfRowColorKey / _depProgramNew / _depContainsNew / _hasNewConc
// helpers were removed. Precedence (in the Python): notincim > new >
// new_partial > launching > completed > inact > change > neutral.
// Map a stored colour key → the row's CSS tint class (notincim is applied
// separately as portfolio-not-in-cim; completed/neutral get no tint).
const _PF_COLOR_CLASS = {
    new: ' row-added', new_partial: ' row-added-partial', launching: ' row-launching',
    inact: ' row-deactivated', change: ' row-edited',
};
...
function getPortfolioFiltered() {
    ...
    // Hide long-retired programs (completed inactivations >3 catalog years old)
    // unless the FOCUS "Show retired" toggle is on.
    if (!portfolioShowRetired)
        rows = rows.filter(p => !_isLongRetired(p));
    if (portfolioLevelFilter.size)
        rows = rows.filter(p => portfolioLevelFilter.has(p.level));
    if (portfolioDegreeFilter.size)
        rows = rows.filter(p => portfolioDegreeFilter.has(p.credential || null));
    ...
    if (portfolioInactTodayFilter)      rows = rows.filter(p => (p.admit_today || '') === portfolioInactTodayFilter);
    if (portfolioInactProgressFilter)   rows = rows.filter(p => (p.cim_inact === 'Y' ? 'Yes' : 'No') === portfolioInactProgressFilter);
    ...
    if (portfolioColorFilter) rows = rows.filter(p => (p.row_color || 'neutral') === portfolioColorFilter);
    ...
    if (portfolioSearch && portfolioSearch.trim()) {
        ...
        // Search the trailing campus/modality parenthetical OFF the name — the
        // normalized `campus` field is matched separately, so a name like
        // "…(Online - Vancouver Requirements)" (campus = Online) must NOT match a
        // "vancouver" search (it's an Online deployment, not a Vancouver one).
        const _searchName = n => (n || '').replace(/\s*\([^)]*\)\s*$/, '');
        ...
        allPortfolioPrograms.forEach(p => {
            ...
            if (p.concentration_of && match(_searchName(p.program_name))) {
                parentIdsViaConc.add(p.concentration_of);
            }
        });
        rows = rows.filter(p =>
            match(_searchName(p.program_name)) ||
            match(p.college) ||
            match(p.campus) ||
            match(p.banner_code || '') ||
            match(p.degree_code || '') ||
...
function renderPortfolioTable() {
    ...
    // parentId -> Set(matching child ids) for concentration-split synth parents
    // pulled in as nesting hosts (below). emitDeploymentRows restricts a promoted
    // host to just these children so a filtered view (e.g. At-risk) shows only the
    // matching concentration nested under its parent, not every sibling.
    const promotedHostConc = {};
    const filteredIds = new Set(filtered.map(r => r.id));
    Object.keys(matchingConcsByParent).forEach(parentId => {
        ...
        } else if (allById[parentId] && String(parentId).startsWith('synth_')) {
            // A concentration-SPLIT synthetic parent (e.g. "Electrical and Computer
            // Engineering, MSECE (Oakland)"): its children ARE the program (each CIM
            // record is "X with Concentration in Y, MSECE"). Nest the matching
            // children under the parent (cleaned concentration names) instead of
            // promoting them standalone with the full "X with Concentration in Y"
            // name, which reads like a bogus program (Waleed 2026-09-22).
            topLevel.push(allById[parentId]);
            topLevelIds.add(parentId);
            promotedHostConc[parentId] = new Set((matchingConcsByParent[parentId] || []).map(c => c.id));
        } else {
            // Any other parent fails the filter — promote children to top-level
            // rows (don't surface an unrelated parent just because a sub-row passes).
    ...
    if (isProgramView) {
        ...
        const _enrCache = {};
        // A group's enrollment sort value = the SUM of the CURRENT enr sort key
        // (e.g. enr_total_2026) across the group's deployment rows AND their linked
        // concentration rows — the SAME rollup the program-header cell displays
        // (_sumEnr). The old version took max latest-year over deployments only,
        // which returned -1 for concentration-split synthetic parents (e.g. Civil
        // Engineering, MSCivE) whose enrollment lives on the concentration rows —
        // so a large program sorted as if it were blank (bug: large programs
        // appeared at the top of an ascending "Total 2026" sort).
        const groupEnrollSort = p => {
            const gk = gkey(p);
            if (gk in _enrCache) return _enrCache[gk];
            const deps = canonicalBanner(p.banner_code) ? (bannerGroups[gk] || [p]) : [p];
            const depIds = new Set(deps.map(d => d.id));
            const enrRows = deps.concat((allPortfolioPrograms || []).filter(
                c => c.concentration_of && depIds.has(c.concentration_of)));
            const key = (portfolioSortKey && portfolioSortKey.indexOf('enr_') === 0)
                ? portfolioSortKey : 'enr_total_2026';
            let any = false, sum = 0;
            enrRows.forEach(r => { const v = _enrValue(r, key); if (typeof v === 'number') { sum += v; any = true; } });
            return (_enrCache[gk] = any ? sum : -Infinity);
        };
        // Required-course count columns are numeric and program-level (same across a
        // program's campuses). Rank a group by the max numeric value across its
        // deployments; blank → -1, matching the flat-view case so the two view modes
        // sort these columns identically.
        const _REQ_FIELD = { reqcore: 'req_core_courses', reqtotal: 'req_total_courses', reqstcount: 'req_st_count' };
        const groupReqSort = (p, field) => {
            const gk = gkey(p);
            const deps = canonicalBanner(p.banner_code) ? (bannerGroups[gk] || [p]) : [p];
            let best = -Infinity;
            deps.forEach(d => { const v = parseFloat(d[field]); if (!isNaN(v)) best = Math.max(best, v); });
        ...
        const gsk = (() => {
            ...
            if (_REQ_FIELD[k]) return 'req';
            return 'name';
        })();
        const gReqField = _REQ_FIELD[portfolioSortKey] || null;
        const gtext = p => {
            switch (gsk) {
                case 'college':     return (p.college || '').toLowerCase();
                case 'credential':  return (p.credential || '').toLowerCase();
        ...
        topLevel.sort((a, b) => {
            let c;
            if (gsk === 'enrollment') {
                // No-enrollment-data groups sort LAST regardless of direction
                // (nulls-last) — so an ascending "Total 2026" surfaces the genuinely
                // lowest-enrolled programs at the top, not the ~dozens with no figure.
                const ea = groupEnrollSort(a), eb = groupEnrollSort(b);
                const na = ea === -Infinity, nb = eb === -Infinity;
                c = (na && nb) ? 0 : na ? 1 : nb ? -1 : (ea - eb) * dir;
            } else if (gsk === 'req') {
                // Numeric required-course counts (matches the flat-view case).
                c = (groupReqSort(a, gReqField) - groupReqSort(b, gReqField)) * dir;
            } else {
                c = gtext(a).localeCompare(gtext(b)) * dir;
            }
            if (c) return c;
    ...
    topLevel.sort((a, b) => {
        ...
        if (portfolioSortKey.startsWith('enr_')) {
            ...
            const ba = isNaN(na), bb = isNaN(nb);   // no-enrollment rows sort LAST (nulls-last)
            if (ba || bb) return ba && bb ? 0 : ba ? 1 : -1;
            return (na - nb) * portfolioSortDir;
        }
        switch (portfolioSortKey) {
            case 'degree':    av = a.credential || ''; bv = b.credential || ''; break;
            ...
            case 'launch':    // GTM Launch is an ISO date (YYYY-MM-DD), not a term — sort as a date; blanks last.
                av = a.roster_launch_date || '9999-99-99'; bv = b.roster_launch_date || '9999-99-99';
                return (av > bv ? 1 : av < bv ? -1 : 0) * portfolioSortDir;
            ...
            case 'otplaunchterm': return ((_pfTermRank(a.otp_launch_term) || -1) - (_pfTermRank(b.otp_launch_term) || -1)) * portfolioSortDir;
            case 'svtnote':   av = a.svtnote || ''; bv = b.svtnote || ''; break;
            case 'cimchange':   av = a.cim_change_type || ''; bv = b.cim_change_type || ''; break;
            case 'ciminact':    av = a.cim_inact === 'Y' ? '1' : '0'; bv = b.cim_inact === 'Y' ? '1' : '0'; break;
            ...
            case 'inacttoday':  av = a.admit_today || ''; bv = b.admit_today || ''; break;
            case 'offering':    av = a.offering || ''; bv = b.offering || ''; break;
            ...
            case 'otpgtmdate':  av = a.otp_gtm_date || '';    bv = b.otp_gtm_date || '';    break;
            ...
            case 'reqcore':     av = parseFloat(a.req_core_courses); bv = parseFloat(b.req_core_courses);
                return ((isNaN(av) ? -1 : av) - (isNaN(bv) ? -1 : bv)) * portfolioSortDir;
            case 'reqtotal':    av = parseFloat(a.req_total_courses); bv = parseFloat(b.req_total_courses);
                return ((isNaN(av) ? -1 : av) - (isNaN(bv) ? -1 : bv)) * portfolioSortDir;
            case 'reqstcount':  av = parseFloat(a.req_st_count); bv = parseFloat(b.req_st_count);
                return ((isNaN(av) ? -1 : av) - (isNaN(bv) ? -1 : bv)) * portfolioSortDir;
            case 'requiresst':   av = a.requires_st || '';     bv = b.requires_st || '';     break;
            case 'reqstcourses': av = a.req_st_courses || '';  bv = b.req_st_courses || '';  break;
    ...
    // Concentration-split parents pulled in as hosts always auto-expand, so the
    // matching concentration is visible nested under the parent (both layouts).
    Object.keys(promotedHostConc).forEach(id => autoExpand.add(id));
    ...
    const emitDeploymentRows = (p, extra) => {
        // Linked concentration sub-rows behave like curriculum concentrations: once
        // the PARENT is shown, show ALL its linked children (subject to the College
        // focus), NOT only children that themselves match the search/filters. A
        // search that matched the parent shouldn't hide the parent's concentrations
        // (the earlier `anyFilterActive ? matchingConcsByParent : allConcsByParent`
        // did exactly that — searching a program hid its idea/linked concentrations).
        // matchingConcsByParent is still used above to PROMOTE a matching child of a
        // NON-shown parent to a standalone top-level row.
        let portfolioConcs = (allConcsByParent[p.id] || [])
            .filter(c => _concPassesCollege(p, (c && c.college) || ''));
        // A concentration-split parent pulled in as a host (parent didn't match the
        // filter, a child did): show ONLY the matching children, so a filtered view
        // doesn't drag in every sibling concentration.
        if (promotedHostConc[p.id]) portfolioConcs = portfolioConcs.filter(c => promotedHostConc[p.id].has(c.id));
        const curriculumConcs = promotedHostConc[p.id] ? [] : (p.concentrations || []);
    ...
    if (isProgramView) {
        ...
        // Built from ALL rows (not just the filtered `topLevel`): a non-CIM
        // deployment must still resolve to its program's Banner even when the
        // CIM (banner-bearing) deployment is filtered out of the current view —
        // otherwise the group loses its Banner identity and the program header
        // opens a name-key no row carries ("No portfolio record", e.g. filtering
        // campus=New York on Media Innovation whose Banner lives on Boston).
        const _nameToBanner = {};
        (allPortfolioPrograms || []).forEach(p => {
        ...
        const gkeyOf = p => {
            const cb = canonicalBanner(p.banner_code);
            if (cb) return cb;                       // CIM row → Banner group (unchanged)
            const bn = _baseKey(p);
            if (bn && _nameToBanner[bn]) return _nameToBanner[bn];  // non-CIM campus of a CIM program → its Banner group (unchanged)
            // Purely non-CIM program: use the server-computed program key so all its
            // campuses share one group AND one program-grain OTP status.
            return p.otp_group_key || (bn ? ('~name:' + bn) : ('~' + p.id));
        };
        const order = [];
        const byG = {};
        topLevel.forEach(p => {
...
function renderPortfolioConcRow(name, search, college, parentCollege, parentCampus, status, svtStatus) {
    ...
    const cellHtml = orderedPortfolioCols()
        ...
        .join('');
    // No status badges on concentration rows (Waleed 2026-09-17): the proposal
    // status is conveyed by the row colours, and the SVT development sticker was
    // both noisy and could be mis-campus'd (an SVT concentration proposal for one
    // campus stamped onto another). `svtStatus` is intentionally unused now.
    return `<tr class="portfolio-row portfolio-curriculum-conc-row">
        <td class="program-name-cell"><span class="pf-caret-spacer"></span>${hl}</td>
        ${cellHtml}
    </tr>`;
}
...
function renderPortfolioProgramRow(gkey, deployments, expanded) {
    ...
    // Program-grain drawer, keyed by the GROUP key: the Banner code for CIM groups
    // (unchanged), or the server-computed name key for a purely-non-CIM program — so
    // a program tracked only in SVT still opens a program record with a shared
    // program-level status.
    const progKey = gkey;
    const nameInner = `<span class="portfolio-record-open pf-open" title="Open program record" onclick="event.stopPropagation(); openProgramRecord('${escapeHtml(progKey)}')">${escapeHtml(name)}</span>`;
    ...
    let dispVals;
    if ((r0.program_dispositions || []).length) {
    ...
    const dispChips = dispVals.length
        ...
        : '';
    // Program row cells open the PROGRAM record by the group key (Banner for CIM,
    // name key for a purely-non-CIM program) to the matching tab.
    const _pOpen = tab => `event.stopPropagation(); openProgramRecord('${escapeHtml(progKey)}','${tab}')`;
    ...
    const pnote = (r0.program_note || '').trim();
    ...
    const _cimInact = deployments.some(d => d.cim_inact === 'Y');
    ...
    const _TERM_FIELD = {
        ...
        otplaunchterm: d => d.otp_launch_term,   // effective (deployment else program grain)
        cimterm:      d => d.cim_term,
        gtmfirst:     d => d.gtm_first_term,
    };
    const vals = {
        degree:      escapeHtml(r0.credential || ''),
        ...
        // Exit master's is a PROGRAM-grain flag (curated by banner code, so every
        // deployment of a program shares it) — surface it on the program header,
        // not just the campus rows. Rolled up (campuses always agree).
        exitmasters: _rollup(d => d.exit_masters),
        // Core-course counts are PROGRAM-grain (joined by subject+degree, so every
        // deployment shares them) — surface on the program header, rolled up.
        reqcore:      _rollup(d => d.req_core_courses),
        reqtotal:     _rollup(d => d.req_total_courses),
        reqstcount:   _rollup(d => d.req_st_count),
        requiresst:   (() => { const v = _rollup(d => d.requires_st);
                                return v === 'Yes' ? '<span class="portfolio-badge badge-bad">Yes</span>' : (v || ''); })(),
        reqstcourses: _rollup(d => d.req_st_courses),
        cim:         _cimCell,
    ...
    // Roll up the stored per-deployment colour keys (p.row_color).
    const _depColor = d => {
        const k = d.row_color || 'neutral';
        return (k === 'notincim' || k === 'inact' || k === 'change') ? k : 'neutral';
    };
    const _allProgNew = deployments.length > 0 && deployments.every(d => (d.row_color) === 'new');
    const _anyNew = deployments.some(d => d.row_color === 'new' || d.row_color === 'new_partial');
...
function renderPortfolioRow(p, opts = {}) {
    ...
    // Program-grain key: Banner (CIM) or the name key (purely non-CIM); fall back to
    // opening the deployment record only when neither exists.
    const _progKey = _bc || p.otp_group_key || '';
    const _progOpen = tab => _progKey
        ? `event.stopPropagation(); openProgramRecord('${escapeHtml(_progKey)}','${tab}')`
        : `event.stopPropagation(); openPortfolioRecord('${escapeHtml(p.id)}','${escapeHtml(p.cim_program_id || '')}',${p.has_regulatory === true},'${tab}')`;
    ...
    // No "Conc." badge on concentration sub-rows (Waleed 2026-09-17): the Credential
    // column already reads "Concentration" and the indentation shows the nesting.
    const concBadge = '';
    ...
    // Row tint from the stored colour key (portfolio_fields.row_color): two-tier
    // green (new / new_partial), purple launching, red inact, blue change — see
    // _PF_COLOR_CLASS. notincim is applied separately as portfolio-not-in-cim.
    const changeClass = _PF_COLOR_CLASS[p.row_color] || '';
    // Synthetic concentration-split parents have no cim_program_id of their own, but
    // their concentration children ARE the CIM records — so don't mark them
    // "Not in CIM" (amber); they use their rolled-up row_color instead.
    const notInCim = (!p.cim_program_id && !isSynthetic) ? ' portfolio-not-in-cim' : '';
    ...
    // Any non-concentration row opens the drawer — CIM-linked rows get the full
    // tab set, non-CIM rows (SVT-added / ideas) get a Summary-only drawer so their
    // deployment-grain OTP disposition & notes can still be edited. Concentrations
    // (curriculum AND idea) are deliberately leaves — NOT clickable (a concentration
    // is a curriculum section, not an independently governed unit). Manage an idea
    // concentration via the "＋ Add idea" modal (Edit/Remove), not the row.
    const _canOpenRec = !isPortfolioConc;
    const nameHtml = _canOpenRec
        ? `<span class="portfolio-record-open pf-open" title="Open program record"
             onclick="event.stopPropagation(); openPortfolioRecord('${escapeHtml(p.id)}','${escapeHtml(p.cim_program_id || '')}',${p.has_regulatory === true})">${escapeHtml(displayName)}</span>`
    ...
    const _cells = {
        degree:        [isPortfolioConc ? 'Concentration' : (p.credential || '')],
        ...
        reqcore:     [escapeHtml(String(p.req_core_courses ?? ''))],
        reqtotal:    [escapeHtml(String(p.req_total_courses ?? ''))],
        requiresst:  [p.requires_st === 'Yes'
            ? '<span class="portfolio-badge badge-bad">Yes</span>'
            : (p.requires_st === 'No' ? 'No' : '')],
        reqstcount:  [escapeHtml(String(p.req_st_count ?? ''))],
        reqstcourses:[escapeHtml(p.req_st_courses || '')],
...
function closeRecordDrawer() {
    // If a course-entry panel is stacked on top, Escape/backdrop closes IT first,
    // leaving the program drawer open underneath.
    const p = document.getElementById('course-entry-panel');
...
function openCimRecord(id) {
    ...
    if (isCourseView) {
        tabKeys = ['workflow'];
        // Catalog Readiness for courses: live locally; on the static site from the
        // baked precheck_courses.json (in-workflow courses).
        if (!window._staticMode || (item && item.current_step)) tabKeys.push('precheck');
    } else {
        ...
        // Catalog Readiness: live on the local app; on the static site it reads the
        // baked precheck.json (in-workflow programs only, which is what's baked).
        if (!window._staticMode || (item && item.current_step)) tabKeys.push('precheck');
    }
    let defaultTab = 'workflow';
    const title = item
        ? (isCourseView ? `${item.code}: ${item.title}` : item.name)
        : ('Program ' + id);
    openRecordDrawer(id, title, tabKeys, defaultTab);
}
...
function openProgramRecord(key, defaultTab) {
    // `key` is the program-grain OTP key: a Banner code (CIM programs) OR the
    // server-computed name key (`~name:subject|degree`) for a purely-non-CIM program.
    key = (key || '').trim();
    _recordDrawerProgramMode = true;
    _recordDrawerBanner = key;
    _recordDrawerPid = null;
    const prog = allPortfolioPrograms.find(p => canonicalBanner(p.banner_code) === key)
              || allPortfolioPrograms.find(p => (p.otp_group_key || '') === key);
    const title = prog ? normalizePortfolioName(stripCampusFromName(prog.program_name)) : ('Program ' + key);
    // Sanitize the DOM id (the key may contain ':', '|', spaces); the real key is
    // kept in _recordDrawerBanner for the OTP save.
    openRecordDrawer('prog_' + key.replace(/[^A-Za-z0-9]+/g, '_'), title, ['summary'], 'summary');
}
function openPortfolioRecord(pid, cimId, hasReg, defaultTab) {
    ...
    // Non-CIM rows (SVT-added programs, manual ideas) have no CIM id, so the
    // curriculum / reference / academic-alignment / regulatory tabs (which fetch by
    // CIM id) don't apply — show ONLY the Summary, whose editable OTP disposition &
    // notes are deployment-grain (keyed on the portfolio row id via _recordDrawerPid),
    // so a non-CIM entry can still be annotated. The drawer is keyed on the portfolio
    // id in that case (a valid, unique key).
    const _cs = (cimId == null) ? '' : String(cimId);
    const hasCim = _cs !== '' && _cs !== 'null' && _cs !== 'undefined';
    ...
    const tabKeys = hasCim ? ['summary', 'curriculum', 'reference', 'academic'] : ['summary'];
    if (hasCim && hasReg) tabKeys.push('regulatory');
    const drawerId = hasCim ? cimId : pid;
    const _dt = (defaultTab === 'otp_disp' || defaultTab === 'otp_notes') ? 'summary' : defaultTab;
    openRecordDrawer(drawerId, prog ? prog.program_name : ('Program ' + drawerId),
                     tabKeys, _dt || 'summary');
}

// Text value shown for a portfolio column key — the SINGLE source of a field's
// display value, shared by the table/CSV export and the record Summary tab.
function colText(p, key, isConc, parent) {
    const ep = isConc && parent ? parent : p;  // effective parent for inherited cols
    switch (key) {
        case 'degree':      return isConc ? 'Concentration' : (p.credential || '');
        case 'bannercode':  return isConc ? '' : (p.banner_code || '');
        ...
        case 'reqcore':      return p.req_core_courses ?? '';
        case 'reqtotal':     return p.req_total_courses ?? '';
        case 'requiresst':   return p.requires_st || '';
        case 'reqstcount':   return p.req_st_count ?? '';
        case 'reqstcourses': return p.req_st_courses || '';
...
function exportPortfolioCsv() {
    ...
    // colText(p,key,isConc,parent) is now module-level (shared with the Summary tab).
...
const _DISP_SHORT = {
    ...
    'OTP recommends inactivation': 'OTP: inactivate',
    'OTP recommends optimization': 'OTP: optimize',
    'OTP recommends merging/consolidating into another program': 'OTP: merging/consolidating',
...
// A row belongs to program-grain key `bc` if its canonical Banner matches (CIM
// rows) OR its otp_group_key matches (purely-non-CIM programs). Mirrors the
// program-grain resolution in database.resolve_otp (pk = banner else group key).
function _programGrainMatch(p, bc) {
    if (!bc) return false;
    return canonicalBanner(p.banner_code) === bc || (p.otp_group_key || '') === bc;
}
...
function _recomputeEffectiveOtp(p) {
    ...
    // OTP First Intake Term and OTP GTM date are DEPLOYMENT-grain only — set
    // directly on the row by their save handlers, so nothing to re-resolve here.
}
...
// Render one OTP editor field (disposition | notes | review) for a grain INTO
// `container`, with container-scoped auto-save. Shared by the Summary tab (which
// is the single edit surface) and the back-compat loadOtpDetail. Dispositions
// save on toggle; notes save on blur / ~0.9s after typing stops. We update the
// in-memory model + table but never re-render this drawer (that would drop
// textarea focus mid-edit).
function _renderOtpField(container, prog, bc, grain, field) {
    if (!container) return;
    const isStatic = typeof window._staticMode !== 'undefined';
    ...
    if (field === 'disposition') {
        ...
        // No inherit hint here — the Summary shows a read-only "Program status"
        // line above the deployment editor, so the inherited value is already visible.
    ...
    } else if (field === 'launchterm') {
        // Deployment grain only — launch timing is per-campus.
        const val = prog.otp_launch_term || '';
        const opts = _pfAllTermValues();
        content = `<input type="text" class="otp-launchterm-input" list="otp-lt-list" value="${escapeHtml(val)}" `
            + `placeholder="e.g. Fall 2027" ${isStatic ? 'readonly' : ''} style="width:180px">`
            + `<datalist id="otp-lt-list">${opts.map(t => `<option value="${escapeHtml(t)}">`).join('')}</datalist>`;
    } else if (field === 'gtmdate') {
        // Deployment grain only. Shows the ingested GTM Roster date as context (this
        // OTP date is a SEPARATE at-risk signal and does not change it).
        const val = prog.otp_gtm_date || '';
        content = `<input type="date" class="otp-gtmdate-input" value="${escapeHtml(val)}" ${isStatic ? 'readonly' : ''} style="width:180px">`;
        const feed = (prog.gtm_date || '').trim();
        if (feed) hint = `<div class="otp-inherit-hint">Ingested GTM Roster date: ${escapeHtml(feed)} (this OTP date is a separate at-risk signal and does not change it).</div>`;
    } else {
    ...
    container.innerHTML = `${content}${hint}${isStatic ? '' : `<div class="otp-save-status" aria-live="polite"></div>`}`;
    if (isStatic) return;
    const sub = field;
    const statusEl = container.querySelector('.otp-save-status');
    let _seq = 0;
    ...
    async function doSave() {
        ...
        try {
            if (sub === 'disposition') {
                const values = [...container.querySelectorAll('.otp-checks input[type=checkbox]:checked')].map(cb => cb.value);
                ...
                const note = (container.querySelector('.otp-note').value || '').trim();
                const url = grain === 'program'
            ...
            } else if (sub === 'launchterm') {
                // Deployment grain only.
                const term = (container.querySelector('.otp-launchterm-input').value || '').trim();
                await fetch(`/api/portfolio/launch-term/${encodeURIComponent(prog.id)}`,
                    {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({term})});
                prog.otp_launch_term = term;
            } else if (sub === 'gtmdate') {
                // Deployment grain only.
                const gtm_date = (container.querySelector('.otp-gtmdate-input').value || '').trim();
                await fetch(`/api/portfolio/gtm-date/${encodeURIComponent(prog.id)}`,
                    {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({gtm_date})});
                prog.otp_gtm_date = gtm_date;
            } else {
                const note = (container.querySelector('.otp-note').value || '').trim();
                const url = grain === 'program'
    ...
    if (sub === 'disposition') {
        container.querySelectorAll('.otp-checks input[type=checkbox]').forEach(cb => cb.addEventListener('change', doSave));
    } else if (sub === 'launchterm') {
        const inp = container.querySelector('.otp-launchterm-input');
        if (inp) {
            let _last = inp.value;
            inp.addEventListener('change', () => { _last = inp.value; doSave(); });   // datalist pick / commit
            inp.addEventListener('blur', () => { if (inp.value !== _last) { _last = inp.value; doSave(); } });
        }
    } else if (sub === 'gtmdate') {
        const inp = container.querySelector('.otp-gtmdate-input');
        if (inp) inp.addEventListener('change', doSave);   // native date picker commit
    } else {
        const ta = container.querySelector('.otp-note');
        if (ta) {
...
// Resolve the drawer's grain + program row + banner, or null if unavailable.
function _drawerOtpContext(drawerId) {
    ...
    if (programMode) {
        ...
        prog = allPortfolioPrograms.find(p => canonicalBanner(p.banner_code) === bc)
            || allPortfolioPrograms.find(p => (p.otp_group_key || '') === bc) || {};
    } else {
    ...
    return { prog, bc, grain: programMode ? 'program' : 'deployment' };
}

// Back-compat standalone OTP tab (no longer wired into any drawer, but kept
// working). OTP editing now lives in the Summary tab (loadSummaryDetail).
function loadOtpDetail(drawerId, field) {
    ...
    const ctx = _drawerOtpContext(drawerId);
    if (!ctx) { el.innerHTML = '<p class="muted" style="padding:8px">No portfolio record.</p>'; return; }
    if (ctx.grain === 'program' && !ctx.bc) {
    ...
    const keyid = ctx.grain === 'program' ? (ctx.bc + ' · all campuses') : (ctx.prog.campus || ctx.prog.id);
    el.innerHTML = `<div class="otp-detail"><section class="otp-scope otp-scope-${ctx.grain}"><div class="otp-scope-head"><span class="otp-scope-key">${escapeHtml(keyid)}</span></div><div id="otp-slot-${drawerId}"></div></section></div>`;
    _renderOtpField(document.getElementById(`otp-slot-${drawerId}`), ctx.prog, ctx.bc, ctx.grain, field);
}

// Summary tab — the single view+edit surface for a program or deployment record.
// Read-only identity (+ per-deployment CIM/SVT/GLS/launch for a deployment) plus
// the EDITABLE OTP disposition/notes for this record's grain. Program records
// show only program-level fields (per-campus data lives in the deployment).
// All values come from colText() so they match the table/CSV exactly.
function loadSummaryDetail(drawerId) {
    const el = document.getElementById(`detail-content-${drawerId}`);
    if (!el) return;
    const ctx = _drawerOtpContext(drawerId);
    if (!ctx || !ctx.prog || (!ctx.prog.program_name && !ctx.prog.id)) {
        el.innerHTML = '<p class="muted" style="padding:8px">No portfolio record.</p>'; return;
    }
    const { prog, bc, grain } = ctx;
    const esc = escapeHtml;
    const row = (label, key) => {
        const v = colText(prog, key);
        return v ? `<div class="sum-row"><span class="sum-k">${esc(label)}</span><span class="sum-v">${esc(v)}</span></div>` : '';
    };
    // Field sections ALWAYS render (so SVT/GTM, GLS, etc. are visible even when a
    // deployment has no feed data): populated rows, else a muted "no data" line —
    // so an empty group reads as "nothing recorded" rather than the group vanishing.
    const section = (title, pairs) => {
        const body = pairs.map(([l, k]) => row(l, k)).join('');
        const inner = body || `<div class="sum-row sum-empty"><span class="sum-v muted">— none recorded —</span></div>`;
        return `<section class="sum-sec"><h4 class="sum-h">${esc(title)}</h4>${inner}</section>`;
    };
    const name = grain === 'program'
        ? normalizePortfolioName(stripCampusFromName(prog.program_name || ''))
        : (prog.program_name || '');
    // Identity — program name row is manual (not a column key).
    const idPairs = [['Credential', 'degree'], ['College', 'college'], ['Banner code', 'bannercode']];
    if (grain === 'deployment') idPairs.splice(1, 0, ['Campus', 'campus']);
    let html = `<section class="sum-sec"><h4 class="sum-h">Identity</h4>`
        + `<div class="sum-row"><span class="sum-k">Program</span><span class="sum-v">${esc(name)}</span></div>`
        + idPairs.map(([l, k]) => row(l, k)).join('') + `</section>`;
    if (grain === 'deployment') {
        html += section('CIM', [['In CIM', 'inworkflow'], ['CIM Step', 'cim'], ['CIM Change', 'cimchange'],
            ['CIM Effective Term', 'cimterm'], ['CIM Catalog', 'cimcatalog'], ['Inactivation in progress', 'ciminact'],
            ['New Offering', 'offering'], ['Inactivation of Admission', 'inactadmit'], ['Admitting Today', 'inacttoday']]);
        html += section('SVT', [['SVT Status', 'svt'], ['SVT Proposal Type', 'svttype'], ['SVT Sub-status', 'substatus'],
            ['Speed to Market', 'speed'], ['SVT Coordination Note', 'svtnote']]);
        html += section('GTM', [['GTM Type', 'gtmtype'], ['GTM Launch date', 'launch'], ['GTM Date', 'gtmdate'],
            ['GTM First Intake', 'gtmfirst'], ['GTM Last Term', 'gtmlast'], ['GTM Intake Terms', 'gtmintake'],
            ['GTM Entered', 'gtmentered']]);
        html += section('Launch terms', [['Proposed Launch Term', 'proposedterm'], ['Expected Launch Term', 'expectedterm']]);
        html += section('GLS', [['GLS Status', 'gls']]);
    }
    // OTP — editable (the single edit surface). Program grain with no Banner code
    // can't carry a program-level OTP; deployment grain always can.
    const g = grain === 'program' ? 'Program' : 'Deployment';
    if (grain === 'program' && !bc) {
        html += `<section class="sum-sec"><h4 class="sum-h">OTP</h4><p class="muted" style="font-size:12px;">No Banner code — program-level OTP is unavailable; set it on a deployment.</p></section>`;
        el.innerHTML = `<div class="summary-detail">${html}</div>`;
        return;
    }
    const _dispChips = arr => (arr || []).length
        ? arr.map(v => `<span class="portfolio-badge disp-badge" title="${esc(v)}">${esc(_dispShort(v))}</span>`).join(' ')
        : `<span class="muted">— none —</span>`;
    // Both grains shown plainly: on a deployment, a read-only PROGRAM status (the
    // shared program-level value it inherits when its own is blank) above the
    // editable DEPLOYMENT status. On a program record, just the editable Program
    // status. (Deployment status overrides the program status; blank = inherit.)
    let otpInner = '';
    // On a deployment, show the inherited read-only PROGRAM status above the editable
    // DEPLOYMENT status. Works for CIM (program grain keyed by Banner) AND purely
    // non-CIM programs (program grain keyed by the name group) — both populate
    // prog.program_dispositions via resolve_otp.
    if (grain === 'deployment') {
        otpInner += `<div class="sum-otp-label">Program status</div>`
                  + `<div class="sum-otp-progval">${_dispChips(prog.program_dispositions || [])}</div>`;
    }
    otpInner += `<div class="sum-otp-label">${g} status</div><div class="otp-detail" id="sum-otpd-${drawerId}"></div>`
              + `<div class="sum-otp-label">${g} notes</div><div class="otp-detail" id="sum-otpn-${drawerId}"></div>`;
    // OTP First Intake Term + OTP GTM Date — DEPLOYMENT grain only (launch timing is
    // per-campus), so they appear only on a deployment record, not a program record.
    if (grain === 'deployment') {
        otpInner += `<div class="sum-otp-label">OTP First Intake Term</div><div class="otp-detail" id="sum-otpl-${drawerId}"></div>`;
        otpInner += `<div class="sum-otp-label">OTP GTM Date</div><div class="otp-detail" id="sum-otpg-${drawerId}"></div>`;
    }
    html += `<section class="sum-sec sum-otp"><h4 class="sum-h">OTP</h4>${otpInner}</section>`;
    el.innerHTML = `<div class="summary-detail">${html}</div>`;
    _renderOtpField(document.getElementById(`sum-otpd-${drawerId}`), prog, bc, grain, 'disposition');
    _renderOtpField(document.getElementById(`sum-otpn-${drawerId}`), prog, bc, grain, 'notes');
    if (grain === 'deployment') {
        _renderOtpField(document.getElementById(`sum-otpl-${drawerId}`), prog, bc, grain, 'launchterm');
        _renderOtpField(document.getElementById(`sum-otpg-${drawerId}`), prog, bc, grain, 'gtmdate');
    }
}
...
// ==================== Portfolio → Courses (catalog browse) ====================
// A browse/find view over the WHOLE Banner course catalog (course_inventory),
// distinct from the CIM Courses view (which is workflow-only). Phase 1: browse +
// filter by subject/college/level/status + search. Phase 2 will attach the
// course→program links (required/elective + campus) from parsed CIM curricula.
let allCatalogCourses = [];
let _catCoursesLoaded = false;
let _catFilters = { q: '', subject: '', college: '', status: 'Active' };
let _catLevelSel = new Set();   // Undergraduate / Graduate, BY COURSE NUMBER (rail Focus buttons)
let _catSort = { key: 'code', dir: 1 };
const _CAT_CAP = 1500;   // cap rendered rows; refine filters to see more

// Link out to the Section Tracker filtered to this course (option A — link, not
// merge; sections are volatile so they stay in their own tracker). Environment-
// aware: the published Courses page → the published Section Tracker; the local app
// → the local Section Tracker. Uses a ?search= deep-link (the Section Tracker
// pre-fills its search box from it; until that param ships there the link still
// opens the Section Tracker, just unfiltered).
function _sectionTrackerUrl(code) {
    const base = window._staticMode ? 'https://wmeleis.github.io/section-tracker/' : 'http://localhost:5055/';
    // term=all so the Section Tracker opens showing this course across EVERY loaded
    // term (not just its Fall-2026 default) — otherwise a course only offered in
    // another term looks "empty" on arrival. The user then picks the term there.
    return base + '?search=' + encodeURIComponent(code || '') + '&term=all';
}

// Course level by NUMBER (per Waleed): 1000-4999 = Undergraduate, 5000+ = Graduate.
// This is number-based, independent of the catalog `level` field.
function _catNumLevel(code) {
    const m = String(code || '').match(/(\d{4})/);
    if (!m) return '';
    return parseInt(m[1], 10) >= 5000 ? 'Graduate' : 'Undergraduate';
}

async function loadPortfolioCourses() {
    const container = document.getElementById('programs-table-container');
    if (!_catCoursesLoaded) {
        if (container) container.innerHTML = '<div class="loading" style="padding:24px;color:#64748b">Loading course catalog…</div>';
        try {
            const res = await fetch(window._staticMode ? 'catalog_courses.json' : '/api/portfolio/courses');
            const data = await res.json();
            allCatalogCourses = Array.isArray(data) ? data : (data.courses || []);
            _catCoursesLoaded = true;
        } catch (e) {
            if (container) container.innerHTML = '<div class="loading" style="padding:24px;color:#b91c1c">Could not load the course catalog.</div>';
            return;
        }
    }
    await _hydrateCourseTeamViews();
    renderPortfolioCourses();
    // Now that the catalog is loaded, refresh the rail so the Subject/College
    // Focus dropdowns + saved Views populate.
    if (window.trackerShell) window.trackerShell.refresh();
}

function _catDistinct(key) {
    return [...new Set(allCatalogCourses.map(c => c[key]).filter(Boolean))].sort();
}
function _setCatFilter(k, v) { _catFilters[k] = v; _renderCatBody(); }
function _catSortBy(key) {
    if (_catSort.key === key) _catSort.dir *= -1; else { _catSort.key = key; _catSort.dir = 1; }
    _renderCatBody();
}
function _catFiltered() {
    const f = _catFilters, q = (f.q || '').trim().toLowerCase();
    let rows = allCatalogCourses.filter(c => {
        if (f.status && f.status !== 'All' && c.status !== f.status) return false;
        if (_catLevelSel.size && !_catLevelSel.has(_catNumLevel(c.code))) return false;
        if (f.subject && c.subject !== f.subject) return false;
        if (f.college && c.college !== f.college) return false;
        if (q && !((c.code + ' ' + (c.title || '') + ' ' + (c.subject || '')).toLowerCase().includes(q))) return false;
        return true;
    });
    const k = _catSort.key, dir = _catSort.dir;
    rows.sort((a, b) => {
        if (k === 'credits') { return ((parseFloat(a.credit_num) || 0) - (parseFloat(b.credit_num) || 0)) * dir; }
...
function _catBarHtml() {
    // Filters (Level buttons + Subject / College / Status) live in the left rail
    // Focus section and search is the header box — consistent with the rest of the
    // app — so the inline bar is just the live result count.
    return '<div class="cat-courses-bar"><span class="cat-count" id="cat-count"></span></div>';
}
function renderPortfolioCourses() {
    const container = document.getElementById('programs-table-container');
    if (!container) return;
    // Bar is persistent (so the search box keeps focus); only the body re-renders.
    container.innerHTML = _catBarHtml() + '<div id="cat-table-wrap"></div>';
    _renderCatBody();
}
function _renderCatBody() {
    const wrap = document.getElementById('cat-table-wrap'); if (!wrap) return;
    const collegeAbbr = (c) => (typeof abbreviateCollege === 'function' ? abbreviateCollege(c) : c);
    const rows = _catFiltered();
    const shown = rows.slice(0, _CAT_CAP);
    const cnt = document.getElementById('cat-count');
    if (cnt) cnt.textContent = `${rows.length.toLocaleString()} course${rows.length === 1 ? '' : 's'}${rows.length > _CAT_CAP ? ` (showing first ${_CAT_CAP} — refine to see more)` : ''}`;
    const th = (key, label) => `<th class="cat-sort" onclick="_catSortBy('${key}')">${label}${_catSort.key === key ? (_catSort.dir > 0 ? ' ▲' : ' ▼') : ''}</th>`;
    let html = '<table class="program-table cat-table"><thead><tr>'
        + th('code', 'Code') + th('title', 'Title') + th('subject', 'Subject')
        + th('college', 'College') + th('level', 'Level') + th('credits', 'Credits')
        + th('status', 'Status') + th('repeat_max', 'Repeat') + th('eff_term', 'Eff term')
        + '<th class="cat-sections-h">Sections</th>'
        + '</tr></thead><tbody>';
    html += shown.map(c => `<tr class="cat-row" onclick="openCatalogCourseDrawer('${escapeHtml(c.code)}')">`
        + `<td class="cat-code">${escapeHtml(c.code)}</td>`
        + `<td>${escapeHtml(c.title)}</td>`
        + `<td>${escapeHtml(c.subject)}</td>`
        + `<td>${escapeHtml(collegeAbbr(c.college))}</td>`
        + `<td>${escapeHtml(c.level)}</td>`
        + `<td>${escapeHtml(c.credits)}</td>`
        + `<td><span class="cat-status cat-status-${c.status === 'Active' ? 'active' : 'inactive'}">${escapeHtml(c.status)}</span></td>`
        + `<td>${escapeHtml(c.repeat_max != null ? String(c.repeat_max) : '')}</td>`
        + `<td>${escapeHtml(c.eff_term)}</td>`
        + `<td class="cat-sections"><a href="${escapeHtml(_sectionTrackerUrl(c.code))}" target="_blank" rel="noopener" onclick="event.stopPropagation()" title="Open this course in the Section Tracker">Sections ↗</a></td>`
        + '</tr>').join('') || '<tr><td colspan="10" class="cat-empty" style="text-align:center;color:#94a3b8;padding:24px">No matching courses.</td></tr>';
    html += '</tbody></table>';
    wrap.innerHTML = html;
}

// Per-course offering history (which terms each course ran), distilled by the Section
// Tracker and shared. Lazy-loaded once, cached; absent → the drawer omits the section.
let _courseOfferings = null;
async function _ensureCourseOfferings() {
    if (_courseOfferings) return _courseOfferings;
    try {
        const res = await fetch(window._staticMode ? 'course_offerings.json' : '/api/portfolio/course-offerings');
        const d = await res.json();
        _courseOfferings = (d && d.courses) || {};
    } catch (e) { _courseOfferings = {}; }
    return _courseOfferings;
}
let _courseDescriptions = null;
async function _ensureCourseDescriptions() {
    if (_courseDescriptions) return _courseDescriptions;
    try {
        const res = await fetch(window._staticMode ? 'course_descriptions.json' : '/api/portfolio/course-descriptions');
        const d = await res.json();
        _courseDescriptions = (d && d.descriptions) || {};
    } catch (e) { _courseDescriptions = {}; }
    return _courseDescriptions;
}

// Course record drawer — catalog details, previous-offerings history (from the Section
// Tracker), + a link out to the Section Tracker for live sections.
// Render a course entry (description + course details + previous offerings + a link
// out to the Section Tracker) into `bodyEl`; `setTitle(text)` sets the container's
// title. Shared by the Courses-tab record drawer AND the stacked panel opened from a
// program curriculum. Lazy-loads the catalog if it isn't loaded yet (cross-view use).
async function _renderCourseEntry(bodyEl, setTitle, code) {
    const esc = escapeHtml;
    if (!allCatalogCourses || !allCatalogCourses.length) {
        try {
            const r = await fetch(window._staticMode ? 'catalog_courses.json' : '/api/portfolio/courses');
            const dd = await r.json();
            allCatalogCourses = Array.isArray(dd) ? dd : (dd.courses || []);
            _catCoursesLoaded = true;
        } catch (e) {}
    }
    const c = (allCatalogCourses || []).find(x => x.code === code);
    if (!c) {
        setTitle(code);
        bodyEl.innerHTML = `<div class="summary-detail"><p class="muted" style="color:#94a3b8;padding:8px">${esc(code)} is not in the current catalog.</p></div>`;
        return;
    }
    setTitle(`${c.code}: ${c.title || ''}`);
    bodyEl.setAttribute('data-course', code);   // guard: only the latest click renders
    const collegeAbbr = (x) => (typeof abbreviateCollege === 'function' ? abbreviateCollege(x) : x);
    const row = (label, val) => (val != null && val !== '')
        ? `<div class="sum-row"><span class="sum-k">${esc(label)}</span><span class="sum-v">${esc(String(val))}</span></div>` : '';
    const button = `<a class="ccd-sections-btn" href="${esc(_sectionTrackerUrl(c.code))}" target="_blank" rel="noopener" title="Open this course in the Section Tracker">Course sections ↗</a>`;
    const courseSection = '<section class="sum-sec"><h4 class="sum-h">Course</h4>'
        + row('Subject', c.subject) + row('College', collegeAbbr(c.college))
        + row('Level (catalog)', c.level) + row('Level (by number)', _catNumLevel(c.code))
        + row('Credits', c.credits) + row('Status', c.status)
        + row('Repeat max', c.repeat_max) + row('Effective term', c.eff_term) + '</section>';
    const loading = '<p class="muted" style="color:#94a3b8;font-size:12px">Loading…</p>';
    bodyEl.innerHTML = '<div class="summary-detail">' + button
        + `<section class="sum-sec"><h4 class="sum-h">Description</h4>${loading}</section>`
        + courseSection
        + `<section class="sum-sec"><h4 class="sum-h">Previous offerings</h4>${loading}</section></div>`;
    const [offerings, descs] = await Promise.all([_ensureCourseOfferings(), _ensureCourseDescriptions()]);
    if (bodyEl.getAttribute('data-course') !== code) return;   // user clicked another course
    const desc = descs[c.code] || '';
    const descSection = desc
        ? `<section class="sum-sec"><h4 class="sum-h">Description</h4><p class="ccd-desc">${esc(desc)}</p></section>` : '';
    const list = offerings[c.code] || [];
    let offHtml;
    if (!list.length) {
        offHtml = '<p class="muted" style="color:#94a3b8;font-size:12px">No offering history on file.</p>';
    } else {
        offHtml = '<div class="ccd-offerings">' + list.map(o => {
            const instr = Array.isArray(o.instructors) ? o.instructors.join(', ') : (o.instructors || '');
            const meta = [o.sections ? `${o.sections} section${o.sections === 1 ? '' : 's'}` : '',
                          (o.enrolled != null && o.enrolled !== '') ? `${o.enrolled} enrolled` : '', instr].filter(Boolean).join(' · ');
            return `<div class="ccd-off-row"><span class="ccd-off-term">${esc(o.term || '')}</span><span class="ccd-off-meta">${esc(meta)}</span></div>`;
        }).join('') + '</div>';
    }
    bodyEl.innerHTML = '<div class="summary-detail">' + button + descSection + courseSection
        + `<section class="sum-sec"><h4 class="sum-h">Previous offerings${list.length ? ` (${list.length})` : ''}</h4>${offHtml}</section></div>`;
}

// Courses-tab row click → the main record drawer.
async function openCatalogCourseDrawer(code) {
    if (!(allCatalogCourses || []).some(x => x.code === code)) return;
    const d = _ensureRecordDrawer();
    d.classList.add('open');
    await _renderCourseEntry(document.getElementById('rec-drawer-body'),
        t => { document.getElementById('rec-drawer-title').textContent = t; }, code);
}

// Course-code click inside a program curriculum → a panel STACKED over the program
// drawer (so closing it returns to the program), not the shared record drawer.
function _ensureCourseEntryPanel() {
    let p = document.getElementById('course-entry-panel');
    if (p) return p;
    p = document.createElement('div');
    p.id = 'course-entry-panel';
    p.innerHTML = `<div class="ce-panel-backdrop" onclick="closeCourseEntryPanel()"></div>
        <div class="ce-panel" role="dialog" aria-label="Course entry">
            <div class="ce-panel-head"><span id="ce-panel-title" class="ce-panel-title"></span>
            <button class="ce-panel-close" title="Close" onclick="closeCourseEntryPanel()">×</button></div>
            <div id="ce-panel-body" class="ce-panel-body"></div>
        </div>`;
    document.body.appendChild(p);
    document.addEventListener('keydown', e => { if (e.key === 'Escape' && p.classList.contains('open')) closeCourseEntryPanel(); });
    return p;
}
function closeCourseEntryPanel() { const p = document.getElementById('course-entry-panel'); if (p) p.classList.remove('open'); }
async function openCourseEntryPanel(code) {
    const p = _ensureCourseEntryPanel();
    p.classList.add('open');
    await _renderCourseEntry(document.getElementById('ce-panel-body'),
        t => { document.getElementById('ce-panel-title').textContent = t; }, code);
}

// ---- Portfolio → Courses: saved Views (personal + team) ----------------------
// Mirrors the Programs Views experience (tiles in the rail + a manage modal +
// personal/team + star), but a "view" is simply the captured flat filter state
// {level, subject, college, status, q, sort} — courses have no nested filter tree.
let courseViewsPersonal = (() => { try { return JSON.parse(localStorage.getItem('cim-course-views-v1')) || []; } catch (e) { return []; } })();
let courseViewsTeam = [];
let _courseViewsTeamLoaded = false;
let _courseViewActiveId = '';
let _courseViewsStarred = (() => { try { return new Set(JSON.parse(localStorage.getItem('cim-course-views-star') || '[]')); } catch (e) { return new Set(); } })();

function _courseViewsAdmin() { return !window._staticMode; }   // team views editable on the local app only
function _persistCourseViewsPersonal() { try { localStorage.setItem('cim-course-views-v1', JSON.stringify(courseViewsPersonal)); } catch (e) {} }
function _persistCourseViewsStar() { try { localStorage.setItem('cim-course-views-star', JSON.stringify([..._courseViewsStarred])); } catch (e) {} }
async function _persistCourseTeamViews() {
    if (window._staticMode) return;   // static site is read-only for team views
    try { await fetch('/api/portfolio/course-views', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ views: courseViewsTeam }) }); } catch (e) {}
}
async function _hydrateCourseTeamViews() {
    if (_courseViewsTeamLoaded) return;
    try {
        if (window._staticMode) { const d = await _getData(); courseViewsTeam = (d && d.course_team_views) || []; }
        else { const r = await fetch('/api/portfolio/course-views'); const j = await r.json(); courseViewsTeam = j.views || []; }
    } catch (e) { courseViewsTeam = []; }
    _courseViewsTeamLoaded = true;
}
function courseViewsAll() {
    const mk = (v, team) => ({ id: v.id, name: v.name, team, starred: _courseViewsStarred.has(v.id), state: v.state });
    return courseViewsTeam.map(v => mk(v, true)).concat(courseViewsPersonal.map(v => mk(v, false)))
        .sort((a, b) => (b.starred ? 1 : 0) - (a.starred ? 1 : 0) || a.name.localeCompare(b.name));
}
function getCourseViewById(id) { return courseViewsAll().find(v => v.id === id); }
function _courseViewState() {
    return { level: [..._catLevelSel], subject: _catFilters.subject, college: _catFilters.college, status: _catFilters.status, q: _catFilters.q, sort: { ..._catSort } };
}
function _applyCourseViewState(s) {
    s = s || {};
    _catLevelSel = new Set(s.level || []);
    _catFilters.subject = s.subject || ''; _catFilters.college = s.college || '';
    _catFilters.status = s.status || 'Active'; _catFilters.q = s.q || '';
    if (s.sort && s.sort.key) _catSort = { key: s.sort.key, dir: s.sort.dir || 1 };
    const hs = document.getElementById('filter-search'); if (hs) hs.value = _catFilters.q;
}
function applyCourseView(id) {
    const v = getCourseViewById(id); if (!v) { _courseViewActiveId = ''; return; }
    _applyCourseViewState(v.state); _courseViewActiveId = id;
    _renderCatBody(); if (window.trackerShell) window.trackerShell.refresh();
}
function clearCourseView() {
    _courseViewActiveId = ''; _catLevelSel = new Set();
    _catFilters = { q: '', subject: '', college: '', status: 'Active' };
    const hs = document.getElementById('filter-search'); if (hs) hs.value = '';
    _renderCatBody(); if (window.trackerShell) window.trackerShell.refresh();
}
function _slugifyView(name) { return (name || 'view').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 40) || 'view'; }
function saveCourseViewFromCurrent(name, team) {
    name = (name || '').trim(); if (!name) return;
    const taken = new Set(courseViewsAll().map(v => v.id));
    const base = (team ? 'team' : 'personal') + '_' + _slugifyView(name);
    let id = base, n = 1; while (taken.has(id)) id = base + '-' + (++n);
    const rec = { id, name, state: _courseViewState() };
    if (team) { courseViewsTeam.push(rec); _persistCourseTeamViews(); } else { courseViewsPersonal.push(rec); _persistCourseViewsPersonal(); }
    _courseViewActiveId = id;
    if (window.trackerShell) window.trackerShell.refresh();
}
function deleteCourseView(id) {
    courseViewsTeam = courseViewsTeam.filter(v => v.id !== id);
    courseViewsPersonal = courseViewsPersonal.filter(v => v.id !== id);
    _persistCourseViewsPersonal(); _persistCourseTeamViews();
    if (_courseViewActiveId === id) _courseViewActiveId = '';
    renderCourseViewsModal(); if (window.trackerShell) window.trackerShell.refresh();
}
function renameCourseView(id) {
    const v = getCourseViewById(id); if (!v) return;
    const name = prompt('Rename view', v.name); if (name == null) return;
    const rec = (v.team ? courseViewsTeam : courseViewsPersonal).find(x => x.id === id);
    if (rec) rec.name = name.trim() || rec.name;
    if (v.team) _persistCourseTeamViews(); else _persistCourseViewsPersonal();
    renderCourseViewsModal(); if (window.trackerShell) window.trackerShell.refresh();
}
function toggleStarCourseView(id) {
    if (_courseViewsStarred.has(id)) _courseViewsStarred.delete(id); else _courseViewsStarred.add(id);
    _persistCourseViewsStar(); renderCourseViewsModal(); if (window.trackerShell) window.trackerShell.refresh();
}
function openCourseViewsModal() {
    _ensureCourseViewsModal();
    document.getElementById('course-views-modal').style.display = 'flex';
    renderCourseViewsModal();
}
function _ensureCourseViewsModal() {
    if (document.getElementById('course-views-modal')) return;
    const el = document.createElement('div');
    el.id = 'course-views-modal'; el.className = 'modal-overlay'; el.style.display = 'none';
    el.onclick = (e) => { if (e.target === el) el.style.display = 'none'; };
    el.innerHTML = '<div class="modal-panel" style="max-width:560px">'
        + '<div class="modal-head"><span class="modal-title">Course Views</span>'
        + '<button class="modal-close" onclick="document.getElementById(\'course-views-modal\').style.display=\'none\'">×</button></div>'
        + '<div id="course-views-body" style="padding:14px 18px 20px"></div></div>';
    document.body.appendChild(el);
}
function renderCourseViewsModal() {
    const body = document.getElementById('course-views-body'); if (!body) return;
    const admin = _courseViewsAdmin();
    const rows = courseViewsAll();
    const listHtml = rows.length ? rows.map(v => {
        const canEdit = v.team ? admin : true;
        return '<div class="cv-row">'
            + `<button class="cv-star${v.starred ? ' on' : ''}" onclick="toggleStarCourseView('${v.id}')" title="Star">${v.starred ? '★' : '☆'}</button>`
            + `<span class="cv-name" onclick="applyCourseView('${v.id}');document.getElementById('course-views-modal').style.display='none'">${escapeHtml(v.name)}</span>`
            + `<span class="cv-tag">${v.team ? 'Team' : 'Personal'}</span>`
            + (canEdit ? `<button class="cv-mini" onclick="renameCourseView('${v.id}')">Rename</button><button class="cv-mini cv-del" onclick="deleteCourseView('${v.id}')">Delete</button>` : '')
            + '</div>';
    }).join('') : '<p style="color:#94a3b8;margin:2px 0 12px">No saved course views yet.</p>';
    body.innerHTML = '<div class="cv-list">' + listHtml + '</div>'
        + '<div class="cv-save"><input id="cv-new-name" type="text" placeholder="Save current filters as…">'
        + (admin ? '<label class="cv-team"><input type="checkbox" id="cv-new-team"> Team</label>' : '')
        + '<button class="cv-save-btn" onclick="_cvSaveCurrent()">Save</button></div>';
}
function _cvSaveCurrent() {
    const name = (document.getElementById('cv-new-name') || {}).value || '';
    if (!name.trim()) return;
    const team = !!(document.getElementById('cv-new-team') || {}).checked;
    saveCourseViewFromCurrent(name, team && _courseViewsAdmin());
    renderCourseViewsModal();
}
...
(function initProgramShell() {
    ...
    initTrackerShell({
        // Brand label reflects the page: CIM Tracker / Portfolio Tracker on the
        // split surfaces, Program Tracker on the combined admin view.
        brand: { label: _isCimSurface() ? 'CIM Tracker' : _isPortfolioSurface() ? 'Portfolio Tracker' : 'Program Tracker', logo: 'N' },
        ...
        // Separate CIM / Portfolio pages (?app=cim | portfolio): each
        // surface shows only its own nav group; the bare URL shows both.
        nav: [].concat(
            _isCimSurface() ? [] : [
                ...
                { label: 'Courses', items: [
                    { label: 'Courses', onSelect: () => switchView('portfolio_courses'), isActive: () => currentView === 'portfolio_courses' },
                ]},
            ],
            _isPortfolioSurface() ? [] : [
                { label: 'CIM', items: [
        ...
        // Admin tools are LOCAL-ONLY: never render them on any published (GitHub
        // Pages) site. Their modals are stripped from the static export and they
        // hit the local Flask API. _staticMode is set before this IIFE runs in the
        // static build (see export_static.build_static_js).
        tools: window._staticMode ? [] : [
        ...
        views: {
            enabled: () => currentView === 'portfolio' || currentView === 'portfolio_courses',
            list: () => {
                if (currentView === 'portfolio_courses')
                    return courseViewsAll().map(v => ({ id: v.id, name: v.name, active: v.id === _courseViewActiveId }));
                return (window._portfolioTileViews || []).map(v => ({ id: v.id, name: v.name, active: v.id === portfolioActiveViewId }));
            },
            apply: id => {
                if (currentView === 'portfolio_courses') { applyCourseView(id); return; }
                applyPortfolioView(id); renderPortfolioTable();
            },
            onManage: () => { if (currentView === 'portfolio_courses') openCourseViewsModal(); else openPortfolioViewsModal(); },
            // Badge text next to the VIEWS heading + the clear-× that resets it.
            activeSummary: () => {
                if (currentView === 'portfolio_courses') { const v = getCourseViewById(_courseViewActiveId); return v ? v.name : ''; }
                const id = portfolioActiveViewId;
                if (!id || id === 'all') return '';
                const v = getPortfolioViewById(id);
                return v ? v.name : '';
            },
            onClear: () => { if (currentView === 'portfolio_courses') { clearCourseView(); return; } applyPortfolioView('all'); renderPortfolioTable(); },
        },
        filters: {
...
function __staticInit() {
    ...
    window.loadCurriculumDetail = async function(programId) {
        ...
        if (html) {
            ...
            contentEl.innerHTML = `<div class="curriculum-content">${cleaned}</div>`;
            if (typeof _linkifyCourseCodes === 'function') _linkifyCourseCodes(contentEl);
        } else {
